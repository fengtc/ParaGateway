...
type Group struct {
	...
	// 该规范化 OpenAI 分组是否只允许 GitHub Copilot OAuth 账号，并恢复旧 Copilot 网关行为
	GithubCopilotOnly bool `json:"github_copilot_only,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldGithubCopilotOnly:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field github_copilot_only", values[i])
			} else if value.Valid {
				_m.GithubCopilotOnly = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("github_copilot_only=")
	builder.WriteString(fmt.Sprintf("%v", _m.GithubCopilotOnly))
	builder.WriteString(", ")
