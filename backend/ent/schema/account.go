...
func (Account) Fields() []ent.Field {
	return []ent.Field{
		...
		// weight: 同一最小优先级内的独立调度权重。
		field.Int("weight").
			Default(100).
			Positive(),

		// rpm_limit: 账号级 60 秒请求硬门槛，独立于 OAuth base_rpm。
		field.Int("rpm_limit").
			Default(0).
			NonNegative(),

		// circuit_breaker_*: 账号连续失败熔断策略。
		field.Int("circuit_breaker_threshold").
			Default(0).
			NonNegative(),
		field.Int("circuit_breaker_cooldown_seconds").
			Default(0).
			NonNegative(),
