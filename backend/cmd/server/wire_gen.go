...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	workDistributionRepository := repository.NewWorkDistributionRepository(db)
	workDistributionService := service.NewWorkDistributionService(workDistributionRepository)
	...
	wifTokenProvider := service.ProvideWIFTokenProvider(secretEncryptor, configConfig)
	claudeTokenProvider := service.ProvideClaudeTokenProvider(accountRepository, geminiTokenCache, oAuthService, oAuthRefreshAPI, wifTokenProvider)
	...
	openAITokenProvider := service.ProvideOpenAITokenProvider(accountRepository, geminiTokenCache, openAIOAuthService, oAuthRefreshAPI, wifTokenProvider)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, openAITokenProvider, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, openAIGatewayService, settingService)
	crsSyncService := service.NewCRSSyncService(accountRepository, proxyRepository, oAuthService, openAIOAuthService, geminiOAuthService, configConfig)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService)
	upstreamAccountRepository := repository.NewUpstreamAccountRepository(db)
	upstreamAccountService := service.NewUpstreamAccountService(upstreamAccountRepository, secretEncryptor, wifTokenProvider, configConfig)
	upstreamAccountHandler := admin.NewUpstreamAccountHandler(upstreamAccountService)
	...
	workDistributionHandler := admin.NewWorkDistributionHandler(workDistributionService)
	...
	requestauditRepository := requestaudit.NewRepository(db)
	requestauditService, err := requestaudit.NewService(requestauditRepository, secretEncryptor, configConfig)
	if err != nil {
		return nil, err
	}
	adminHandler := requestaudit.NewAdminHandler(requestauditService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, upstreamAccountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, workDistributionHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, adminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, requestauditService, workDistributionService, idempotencyCoordinator, idempotencyCleanupService)
