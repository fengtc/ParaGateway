...
func (h *OpenAIGatewayHandler) GrokRealtime(c *gin.Context) {
	...
	if slotStatus == openAISlotAcquireRuntimeRejected {
		h.errorResponse(c, http.StatusTooManyRequests, "rate_limit_error", "Grok account is temporarily unavailable")
		return
	}
	...
	outcome := service.AccountRuntimeOutcomeFromContext(c.Request.Context(), proxyErr)
	if proxyErr != nil && isExpectedGrokRealtimeClose(proxyErr) {
		outcome = service.AccountRuntimeOutcomeHealthy
	}
	h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, model, outcome, nil)
	if proxyErr != nil {
...
func (h *OpenAIGatewayHandler) GrokVoice(c *gin.Context, endpoint string) {
	...
	for attempts := 0; attempts < 4; attempts++ {
		...
		if status == openAISlotAcquireRuntimeRejected {
		...
		h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, selectionModel, service.AccountRuntimeOutcomeFromOpenAIForwardContext(c.Request.Context(), result, forwardErr), nil)
		if forwardErr == nil {
