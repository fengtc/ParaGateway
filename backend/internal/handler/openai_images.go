...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &imageUpstreamErr) {
					...
					upstreamBody := []byte(imageUpstreamErr.Code + " " + imageUpstreamErr.Message)
					h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(requestModel), service.AccountRuntimeOutcomeFromUpstreamStatus(imageUpstreamErr.StatusCode, upstreamBody), nil)
				...
				if errors.As(err, &failoverErr) {
					h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(requestModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), failoverErr), nil)
				...
				h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(requestModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), err), nil)
		...
		h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(requestModel), service.AccountRuntimeOutcomeFromOpenAIForwardContext(c.Request.Context(), result, err), firstTokenMs)
