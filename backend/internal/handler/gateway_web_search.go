...
func (h *GatewayHandler) WebSearch(c *gin.Context) {
	...
	for attempt := 0; attempt < 4; attempt++ {
		...
		h.gatewayService.RecordAccountSelectionRuntimeOutcome(c.Request.Context(), selected, service.AccountRuntimeOutcomeFromContext(c.Request.Context(), err))
		if err == nil {
			break
		}
...
func (h *GatewayHandler) acquireWebSearchAccountSlot(
	...
	gate, gateErr := h.gatewayService.AcquireAccountSelectionRuntimePolicy(c.Request.Context(), selected)
	if gateErr != nil {
		if slotRelease != nil {
			slotRelease()
		}
