	...
	for {
		...
		if !selection.RuntimePolicyAdmitted {
			...
			if gateErr != nil {
				...
				reqLog.Warn("gateway.responses.account_runtime_gate_failed", zap.Int64("account_id", account.ID), zap.Error(gateErr))
				h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Account runtime policy unavailable")
				return
			}
			if !gate.Allowed {
				if accountReleaseFunc != nil {
