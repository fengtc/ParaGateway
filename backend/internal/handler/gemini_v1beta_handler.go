...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	selection, allowed, gateErr := h.admitAIStudioAccountRuntimePolicy(c.Request.Context(), account)
	if gateErr != nil {
		googleError(c, http.StatusServiceUnavailable, "Account runtime policy unavailable")
		return
	}
	if !allowed {
		// 模型目录本身已有静态回退；RPM 或熔断拒绝时不再触碰该上游账号。
		c.JSON(http.StatusOK, gemini.FallbackModelsList())
		return
	}

	res, err := h.geminiCompatService.ForwardAIStudioGET(c.Request.Context(), account, "/v1beta/models")
	h.gatewayService.RecordAccountSelectionRuntimeOutcome(
		c.Request.Context(),
		selection,
		aiStudioAccountRuntimeOutcome(c.Request.Context(), res, err),
	)
	if err != nil {
...
func (h *GatewayHandler) GeminiV1BetaGetModel(c *gin.Context) {
	...
	if !allowed {
		// 模型详情同样可由本地目录安全回退，避免命中已开路/超 RPM 的账号。
		c.JSON(http.StatusOK, gemini.FallbackModel(modelName))
		return
	}
...
// admitAIStudioAccountRuntimePolicy applies the same per-account RPM/circuit
// policy as inference endpoints. AI Studio model discovery has no concurrency
// lease, so its selection is considered admitted immediately before the network
// request and the returned selection carries the request-scoped circuit epoch.
func (h *GatewayHandler) admitAIStudioAccountRuntimePolicy(ctx context.Context, account *service.Account) (*service.AccountSelectionResult, bool, error) {
	selection := &service.AccountSelectionResult{Account: account, Acquired: true}
	gate, err := h.gatewayService.AcquireAccountSelectionRuntimePolicy(ctx, selection)
	if err != nil {
		return selection, false, err
	}
	return selection, gate.Allowed, nil
}

func aiStudioAccountRuntimeOutcome(ctx context.Context, result *service.UpstreamHTTPResult, err error) service.AccountRuntimeOutcome {
	if err != nil {
		return service.AccountRuntimeOutcomeFromContext(ctx, err)
	}
	if result == nil {
		return service.AccountRuntimeOutcomeNeutral
	}
	return service.AccountRuntimeOutcomeFromUpstreamStatus(result.StatusCode, result.Body)
}
	...
	for {
		...
		if !selection.RuntimePolicyAdmitted {
			...
			if gateErr != nil {
				...
				reqLog.Warn("gemini.account_runtime_gate_failed", zap.Int64("account_id", account.ID), zap.Error(gateErr))
				googleError(c, http.StatusServiceUnavailable, "Account runtime policy unavailable")
				return
			}
			if !gate.Allowed {
