...
type CreateAccountRequest struct {
	...
	// Worker 风格的权重、RPM 与熔断策略属于独立 upstream_accounts 领域。
	// 字段暂留在内部服务输入中用于兼容既有数据库，但官方账号 API 不再接收它们。
	Weight                        int      `json:"-"`
	RPMLimit                      int      `json:"-"`
	CircuitBreakerThreshold       int      `json:"-"`
	CircuitBreakerCooldownSeconds int      `json:"-"`
...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	if containsIndependentUpstreamAuth(req.Extra, req.Credentials) {
		response.BadRequest(c, "WIF 与 Worker 风格上游认证必须在独立的上游账号页面配置")
		return
	}
	...
	result, err := executeAdminIdempotent(c, "admin.accounts.create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		account, execErr := h.adminService.CreateAccount(ctx, &service.CreateAccountInput{
			...
			CircuitBreakerCooldownSeconds: req.CircuitBreakerCooldownSeconds,
	...
	executeAdminIdempotentJSON(c, "admin.accounts.batch_create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		...
		for _, item := range req.Accounts {
			...
			account, err := h.adminService.CreateAccount(ctx, &service.CreateAccountInput{
				...
				CircuitBreakerThreshold:       item.CircuitBreakerThreshold,
				CircuitBreakerCooldownSeconds: item.CircuitBreakerCooldownSeconds,
...
func (h *AccountHandler) BulkUpdate(c *gin.Context) {
	...
	hasUpdates := req.Name != "" ||
		...
		req.CircuitBreakerCooldownSeconds != nil ||
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	if account.IsOpenAI() {
		// GitHub Copilot exposes an OpenAI-compatible Chat Completions catalog,
		// not ChatGPT/Codex's internal model list. Prefer the Worker-compatible
		// fallback catalog here; the explicit sync action can replace it with a
		// live /v1/models result when the account allows discovery.
		if account.IsGitHubCopilot() {
			models := make([]openai.Model, 0, len(service.CopilotDefaultModels()))
			for _, modelID := range service.CopilotDefaultModels() {
...
// containsIndependentUpstreamAuth prevents the official accounts API from
// becoming a second write path for the independent upstream_accounts domain.
// Existing legacy rows remain readable so this correction never deletes data.
func containsIndependentUpstreamAuth(extra, credentials map[string]any) bool {
	if authType, ok := extra["auth_type"].(string); ok && strings.EqualFold(strings.TrimSpace(authType), service.UpstreamAuthWIF) {
		return true
	}
	for key := range extra {
		if strings.HasPrefix(strings.ToLower(strings.TrimSpace(key)), "wif_") {
			return true
		}
	}
	for key := range credentials {
		normalized := strings.ToLower(strings.TrimSpace(key))
		if strings.HasPrefix(normalized, "wif_") || normalized == "wif_client_secret" {
			return true
		}
	}
	return false
