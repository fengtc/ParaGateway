...
type CopilotOAuthPollRequest struct {
	FlowID string `uri:"id" binding:"required"`
}
...
// StartCopilotOAuth starts GitHub Copilot's device authorization flow.
// POST /api/v1/admin/openai/copilot/flows
func (h *OpenAIOAuthHandler) StartCopilotOAuth(c *gin.Context) {
	var req CopilotOAuthStartRequest
	if err := c.ShouldBindJSON(&req); err != nil {
	...
	response.Created(c, copilotOAuthFlowResponse(result))
}

// PollCopilotOAuth advances a device authorization flow. The callback creates
// the account on the server so no OAuth token reaches the browser.
// POST /api/v1/admin/openai/copilot/flows/:id/poll
func (h *OpenAIOAuthHandler) PollCopilotOAuth(c *gin.Context) {
...
func copilotOAuthFlowResponse(result *service.CopilotOAuthFlowResult) *CopilotOAuthFlowResponse {
	...
	return &CopilotOAuthFlowResponse{
		...
		Provider:          dto.AccountFromService(result.ProviderAccount),
	}
}
