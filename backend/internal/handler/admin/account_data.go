...
type DataPayload struct {
	...
	// WIF 密文与实例级加密密钥绑定，不能安全跨实例恢复。导出时整条排除，
	// 避免泄露密文或生成看似可恢复、实际必坏的备份。
	SkippedWIFAccounts int `json:"skipped_wif_accounts,omitempty"`
}
...
func (h *AccountHandler) ExportData(c *gin.Context) {
	...
	if skippedWIFAccounts > 0 {
		slog.Warn("export_skipped_wif_accounts", "count", skippedWIFAccounts, "reason", "instance-bound ciphertext is not portable")
	}
	...
	for i := range accounts {
		...
		dataAccounts = append(dataAccounts, DataAccount{
			...
			Weight:                        acc.EffectiveWeight(),
			RPMLimit:                      acc.EffectiveRPMLimit(),
			CircuitBreakerThreshold:       acc.EffectiveCircuitBreakerThreshold(),
			CircuitBreakerCooldownSeconds: int(acc.EffectiveCircuitBreakerCooldown() / time.Second),
	...
	payload := DataPayload{
		...
		SkippedWIFAccounts: skippedWIFAccounts,
	}
...
func validateDataAccount(item DataAccount) error {
	...
	if (&service.Account{Platform: item.Platform, Type: item.Type, Extra: item.Extra}).IsWIF() {
		return errors.New("WIF accounts cannot be restored from data backup; recreate the account and provide the client secret on this instance")
	}
	weight := item.Weight
	if weight == 0 {
		weight = 100 // 旧版备份未包含 weight。
	}
	if weight <= 0 || weight > 1_000_000 {
		return errors.New("weight must be between 1 and 1000000")
	}
	if item.RPMLimit < 0 || item.RPMLimit > 1_000_000 {
		return errors.New("rpm_limit must be between 0 and 1000000")
	}
	if item.CircuitBreakerThreshold < 0 || item.CircuitBreakerThreshold > 1000 {
		return errors.New("circuit_breaker_threshold must be between 0 and 1000")
	}
	if item.CircuitBreakerCooldownSeconds < 0 || item.CircuitBreakerCooldownSeconds > 86400 {
