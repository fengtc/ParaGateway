...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		if err == nil {
			h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(requestedModel), service.AccountRuntimeOutcomeHealthy, nil)
			if result != nil {
		...
		if !errors.As(err, &failoverErr) {
			h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(requestedModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), err), nil)
		...
		h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(requestedModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), failoverErr), nil)
		if c.Writer.Size() != writerSizeBeforeForward {
