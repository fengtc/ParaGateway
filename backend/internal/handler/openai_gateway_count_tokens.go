...
func (h *OpenAIGatewayHandler) CountTokens(c *gin.Context) {
	...
	if slotStatus == openAISlotAcquireRuntimeRejected {
		h.anthropicErrorResponse(c, http.StatusTooManyRequests, "rate_limit_error", "Account request limit reached, please retry later")
		return
	}
	if slotStatus == openAISlotAcquireProfitVetoed {
		// count_tokens 已显式 suppress 利润门；保留防御性错误，避免无响应返回。
	...
	h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(currentRoutingModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), forwardErr), nil)
	if forwardErr != nil {
