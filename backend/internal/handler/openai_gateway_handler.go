...
func usageRecordContext(parent context.Context, base context.Context) context.Context {
	...
	if attribution, ok := service.UsageWorkAttributionFromContext(parent); ok {
		base = service.WithUsageWorkAttribution(base, attribution)
	}
	return base
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		if slotResult == openAISlotAcquireRuntimeRejected {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(reqModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), failoverErr), nil)
				...
				h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(reqModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), err), nil)
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(reqModel), service.AccountRuntimeOutcomeFromOpenAIForwardContext(c.Request.Context(), result, err), result.FirstTokenMs)
		} else {
			h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(reqModel), service.AccountRuntimeOutcomeFromOpenAIForwardContext(c.Request.Context(), result, err), nil)
		}
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(currentRoutingModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), failoverErr), nil)
				...
				h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(currentRoutingModel), service.AccountRuntimeOutcomeFromContext(c.Request.Context(), err), nil)
		...
		h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(currentRoutingModel), service.AccountRuntimeOutcomeFromOpenAIForwardContext(c.Request.Context(), result, err), firstTokenMs)
...
const (
	...
	// openAISlotAcquireRuntimeRejected：WaitPlan 已取得槽位，但此时 RPM/熔断
	// 原子准入拒绝。槽位已释放且未写响应；调用方应排除该账号后重新选号。
	openAISlotAcquireRuntimeRejected
)
...
func (h *OpenAIGatewayHandler) acquireResponsesAccountSlot(
	...
	account := selection.Account
	admitRuntimePolicy := func(release func()) openAISlotAcquireResult {
		gate, gateErr := h.gatewayService.AcquireAccountSelectionRuntimePolicy(ctx, selection)
		if gateErr != nil {
			if release != nil {
				release()
			}
			reqLog.Warn("openai.account_runtime_gate_failed", zap.Int64("account_id", account.ID), zap.Error(gateErr))
			h.handleStreamingAwareError(c, http.StatusServiceUnavailable, "api_error", "Account runtime policy unavailable", *streamStarted)
			return openAISlotAcquireFailed
		}
		if !gate.Allowed {
			if release != nil {
				release()
			}
			reqLog.Debug("openai.account_runtime_gate_rejected",
				zap.Int64("account_id", account.ID),
				zap.String("reason", string(gate.Reason)),
				zap.Duration("retry_after", gate.RetryAfter),
			)
			return openAISlotAcquireRuntimeRejected
		}
		return openAISlotAcquireOK
	}
	if selection.Acquired {
		if status := admitRuntimePolicy(selection.ReleaseFunc); status != openAISlotAcquireOK {
			return nil, status
		}
	...
	if fastAcquired {
		if status := admitRuntimePolicy(fastReleaseFunc); status != openAISlotAcquireOK {
			return nil, status
		}
	...
	releaseWait()
	if status := admitRuntimePolicy(accountReleaseFunc); status != openAISlotAcquireOK {
		return nil, status
	}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	var lastFailoverErr *service.UpstreamFailoverError
	var oauth429FailoverState service.OpenAIOAuth429FailoverState
	var activeSelection *service.AccountSelectionResult
	wsAttemptMessage := append([]byte(nil), firstMessage...)
	handleWSFailover := func(account *service.Account, failoverErr *service.UpstreamFailoverError) bool {
		if ctx.Err() != nil {
			return false
		}
		h.gatewayService.ReportOpenAIAccountSelectionOutcome(activeSelection, account.GetMappedModel(reqModel), service.AccountRuntimeOutcomeFromContext(ctx, failoverErr), nil)
		releaseAccountSlot()
		if !failoverErr.ShouldRetryNextAccount() {
	...
	for {
		...
		if selection.Acquired {
			gate, gateErr := h.gatewayService.AcquireAccountSelectionRuntimePolicy(admissionCtx, selection)
			if gateErr != nil {
				if accountReleaseFunc != nil {
					accountReleaseFunc()
				}
				reqLog.Warn("openai.websocket_account_runtime_gate_failed", zap.Int64("account_id", account.ID), zap.Error(gateErr))
				closeOpenAIClientWS(wsConn, coderws.StatusInternalError, "account runtime policy unavailable")
				return
			}
			if !gate.Allowed {
				if accountReleaseFunc != nil {
		...
		var runtimeTurnSelection atomic.Pointer[service.AccountSelectionResult]
		runtimeTurnSelection.Store(selection)
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeTurn: func(turn int) error {
				...
				turnSelection := &service.AccountSelectionResult{Account: account, Acquired: true}
				gate, gateErr := h.gatewayService.AcquireAccountSelectionRuntimePolicy(turnCtx, turnSelection)
				if gateErr != nil || !gate.Allowed {
					...
					if gateErr != nil {
						return service.NewOpenAIWSClientCloseError(coderws.StatusInternalError, "account runtime policy unavailable", gateErr)
					}
					return service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "account request limit reached, please reconnect", nil)
				}
				runtimeTurnSelection.Store(turnSelection)
				currentUserRelease = wrapReleaseOnDone(ctx, userReleaseFunc)
				currentAccountRelease = wrapReleaseOnDone(ctx, accountReleaseFunc)
				return nil
			},
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				if turnErr != nil {
					if result == nil || result.ImageCount <= 0 {
						reportSelection := runtimeTurnSelection.Load()
						h.gatewayService.ReportOpenAIAccountSelectionOutcome(reportSelection, account.GetMappedModel(turnRequestedModel), service.AccountRuntimeOutcomeFromOpenAIForwardContext(ctx, result, turnErr), nil)
						return
					}
				...
				h.gatewayService.ReportOpenAIAccountSelectionOutcome(runtimeTurnSelection.Load(), scheduleModel, service.AccountRuntimeOutcomeFromOpenAIForwardContext(ctx, result, turnErr), result.FirstTokenMs)
				inboundEndpoint := GetInboundEndpoint(c)
		...
		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, wsFirstMessage, hooks); err != nil {
			...
			if shouldReportOpenAIWSProxyAccountFailure(err) {
				h.gatewayService.ReportOpenAIAccountSelectionOutcome(selection, account.GetMappedModel(reqModel), service.AccountRuntimeOutcomeFromContext(ctx, err), nil)
			}
			closeStatus, closeReason := summarizeWSCloseErrorForLog(err)
