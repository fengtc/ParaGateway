...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			// WaitPlan 只有在真正取得并发槽后才执行 RPM/熔断原子准入；
			// 排队超时、取消或队列已满不会消耗 RPM。
			if !selection.RuntimePolicyAdmitted {
				...
				if gateErr != nil {
					...
					reqLog.Warn("gateway.account_runtime_gate_failed", zap.Int64("account_id", account.ID), zap.Error(gateErr))
