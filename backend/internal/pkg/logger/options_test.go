...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/paragateway-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/paragateway-data", "logs", "paragateway.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "paragateway.log")
