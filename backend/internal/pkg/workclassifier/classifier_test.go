...
func TestClassifyTransientText(t *testing.T) {
	...
	for _, test := range tests {
		...
		if result.Category != test.category {
			t.Fatalf("%q: got %s, want %s", test.text, result.Category, test.category)
		}
		if result.ClassificationSource != SourceLocalRule {
			t.Fatalf("unexpected source %s", result.ClassificationSource)
		}
	}
}
...
func TestClassifyUnknownTextAsUnclassified(t *testing.T) {
	...
	if result.Category != CategoryUnclassified || result.WorkRelated != WorkRelationUncertain {
