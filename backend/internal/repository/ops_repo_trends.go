...
ORDER BY bucket ASC`
	...
	if platform == "" && (groupID == nil || *groupID <= 0) {
		items, err := r.getThroughputBreakdownByPlatform(ctx, start, end, filter)
		if err != nil {
			return nil, err
		}
		byPlatform = items
	} else if platform != "" && (groupID == nil || *groupID <= 0) {
		items, err := r.getThroughputTopGroupsByPlatform(ctx, start, end, platform, 10, filter)
		if err != nil {
			return nil, err
		}
		topGroups = items
...
func (r *opsRepository) getThroughputBreakdownByPlatform(ctx context.Context, start, end time.Time, filter *service.OpsDashboardFilter) ([]*service.OpsThroughputPlatformBreakdownItem, error) {
	usageExtra, errorExtra, extraArgs, _ := opsEntityFilterSQL(filter, "ul.", "", 3)
	q := `
...
func (r *opsRepository) getThroughputTopGroupsByPlatform(ctx context.Context, start, end time.Time, platform string, limit int, filter *service.OpsDashboardFilter) ([]*service.OpsThroughputGroupBreakdownItem, error) {
	...
	usageExtra, errorExtra, extraArgs, next := opsEntityFilterSQL(filter, "ul.", "", 4)
	limitArg := next
	args := []any{start, end, platform}
	args = append(args, extraArgs...)
	args = append(args, limit)
	limitClause := fmt.Sprintf("LIMIT $%d", limitArg)
