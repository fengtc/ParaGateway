...
// ListExternalUsageLogs implements the legacy denormalized external usage export.
func (r *usageLogRepository) ListExternalUsageLogs(ctx context.Context, params pagination.PaginationParams, filters service.ExternalUsageLogFilters) ([]service.ExternalUsageLog, *pagination.PaginationResult, *service.ExternalUsageLogTotals, error) {
	...
	if model := strings.TrimSpace(filters.Model); model != "" {
		...
		conditions = append(conditions, fmt.Sprintf("(ul.model = $%d OR ul.requested_model = $%d OR ul.upstream_model = $%d)", placeholder, placeholder, placeholder))
	...
	items := make([]service.ExternalUsageLog, 0, pageSize)
	for rows.Next() {
	...
	return items, pageResult, totals, nil
}
