...
func createAccountRecord(ctx context.Context, client *dbent.Client, account *service.Account) error {
	...
	builder := client.Account.Create().
		...
		SetWeight(account.EffectiveWeight()).
		SetRpmLimit(account.EffectiveRPMLimit()).
		SetCircuitBreakerThreshold(account.EffectiveCircuitBreakerThreshold()).
		SetCircuitBreakerCooldownSeconds(int(account.EffectiveCircuitBreakerCooldown() / time.Second)).
...
func (r *accountRepository) ClearTempUnschedulableUnlessReason(ctx context.Context, id int64, protectedReason string) (bool, error) {
	result, err := r.sql.ExecContext(ctx, `
		UPDATE accounts
		SET temp_unschedulable_until = NULL,
			temp_unschedulable_reason = NULL,
		...
		WHERE id = $1
			AND deleted_at IS NULL
			AND COALESCE(temp_unschedulable_reason, '') <> $2
	`, id, protectedReason)
	if err != nil {
		return false, err
	}
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	if updates.CircuitBreakerThreshold != nil {
		setClauses = append(setClauses, "circuit_breaker_threshold = $"+itoa(idx))
		args = append(args, *updates.CircuitBreakerThreshold)
		idx++
	}
	if updates.CircuitBreakerCooldownSeconds != nil {
		setClauses = append(setClauses, "circuit_breaker_cooldown_seconds = $"+itoa(idx))
		args = append(args, *updates.CircuitBreakerCooldownSeconds)
		idx++
	}
...
func accountEntityToService(m *dbent.Account) *service.Account {
	...
	return &service.Account{
		...
		CircuitBreakerCooldownSeconds: m.CircuitBreakerCooldownSeconds,
