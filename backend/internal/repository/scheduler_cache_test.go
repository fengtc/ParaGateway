...
func TestSchedulerMetadataAccountKeepsRuntimeSchedulingPolicy(t *testing.T) {
	account := service.Account{
		...
		CircuitBreakerCooldownSeconds: 135,
	}

	metadata := buildSchedulerMetadataAccount(account)
	require.Equal(t, account.Weight, metadata.Weight)
	require.Equal(t, account.RPMLimit, metadata.RPMLimit)
	require.Equal(t, account.CircuitBreakerThreshold, metadata.CircuitBreakerThreshold)
	require.Equal(t, account.CircuitBreakerCooldownSeconds, metadata.CircuitBreakerCooldownSeconds)
}
