...
func (s *GatewayService) handleErrorResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, requestedModel ...string) (*ForwardResult, error) {
	...
	if status, errType, errMsg, matched := applyErrorPassthroughRule(
		...
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, body, "upstream error: %d (passthrough rule matched) message=%s", resp.StatusCode, summary)
	}
	...
	case 400:
		...
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, body, "upstream error: %d message=%s", resp.StatusCode, summary)
	case 401:
...
func (s *GatewayService) handleRetryExhaustedError(ctx context.Context, resp *http.Response, c *gin.Context, account *Account) (*ForwardResult, error) {
	...
	if status, errType, errMsg, matched := applyErrorPassthroughRule(
		...
		if summary == "" {
			return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream error: %d (retries exhausted, passthrough rule matched)", resp.StatusCode)
		}
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream error: %d (retries exhausted, passthrough rule matched) message=%s", resp.StatusCode, summary)
	}
	...
	if upstreamMsg == "" {
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream error: %d (retries exhausted)", resp.StatusCode)
	}
	return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream error: %d (retries exhausted) message=%s", resp.StatusCode, upstreamMsg)
}
