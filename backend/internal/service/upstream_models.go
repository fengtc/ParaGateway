...
func (s *AccountTestService) FetchUpstreamSupportedModels(ctx context.Context, account *Account) ([]string, error) {
	...
	// Copilot's catalog is account-dependent and its models endpoint can be
	// temporarily unavailable while a freshly exchanged token propagates. The
	// Worker implementation deliberately falls back to a known-compatible
	// catalog in that case; keep the admin sync action useful on standalone hosts
	// as well, without masking errors for other providers.
...
func (s *AccountTestService) fetchUpstreamSupportedModels(ctx context.Context, account *Account) ([]string, error) {
	...
	if account.IsGitHubCopilot() {
		for i, model := range models {
			models[i] = copilotModelForClient(model)
		}
...
func (s *AccountTestService) buildAnthropicUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	...
	if account.IsOAuth() {
		...
		apiKey, tokenErr := s.resolveAnthropicAPIKeyToken(ctx, account)
		if tokenErr != nil {
			return nil, newUpstreamModelSyncUpstreamError("Failed to resolve Anthropic credential", tokenErr)
		}
...
func (s *AccountTestService) buildOpenAIUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	if account != nil && account.IsGitHubCopilot() {
		if account.Type != AccountTypeOAuth {
			return nil, newUpstreamModelSyncUnsupportedError("GitHub Copilot requires an OAuth account", nil)
		}
		accessToken := ""
		if s.openAITokenProvider != nil {
			...
			if err != nil {
				return nil, newUpstreamModelSyncUpstreamError("Failed to get GitHub Copilot access token", err)
			}
		}
		...
		if strings.TrimSpace(accessToken) == "" {
			return nil, newUpstreamModelSyncConfigError("No GitHub Copilot access token is available", nil)
		}
		...
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, buildCopilotAPIURL(normalizedBaseURL, "/models"), nil)
		if err != nil {
			return nil, newUpstreamModelSyncConfigError("Invalid GitHub Copilot model list URL", err)
		}
		for key, value := range copilotHeaders(accessToken, false) {
			req.Header.Set(key, value)
		}
		account.ApplyHeaderOverrides(req.Header)
		return req, nil
	...
	apiKey, tokenErr := s.resolveOpenAIAPIKeyToken(ctx, account)
	if tokenErr != nil {
		return nil, newUpstreamModelSyncUpstreamError("Failed to resolve OpenAI credential", tokenErr)
	}
	apiKey = strings.TrimSpace(apiKey)
	if apiKey == "" {
