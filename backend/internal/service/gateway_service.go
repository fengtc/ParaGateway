...
// UpstreamHTTPStatusError preserves a non-failover upstream HTTP response for
// runtime health classification. Callers may already have written the response
// to the client; unlike UpstreamFailoverError this type does not request an
// account switch, it only carries status/body without brittle string parsing.
...
func newUpstreamHTTPStatusError(statusCode int, responseBody []byte, format string, args ...any) error {
...
func newRequestScopedUpstreamHTTPStatusError(statusCode int, responseBody []byte, format string, args ...any) error {
	err := newUpstreamHTTPStatusError(statusCode, responseBody, format, args...)
	err.(*UpstreamHTTPStatusError).Scope = GatewayFailureScopeRequest
	return err
}

// UpstreamTransportError 对客户端只暴露脱敏后的消息，同时通过 Unwrap 保留
// 原始网络错误类型，供 runtime policy 区分真实上游网络失败与本地转换错误。
...
func newUpstreamTransportError(cause error, safeMessage string) error {
	return &UpstreamTransportError{
		Message: "upstream request failed: " + sanitizeUpstreamErrorMessage(safeMessage),
		Cause:   cause,
	}
}
	...
	case AccountTypeAPIKey:
		if account.IsWIF() {
			if s.claudeTokenProvider == nil {
				return "", "", errors.New("WIF token provider not configured")
			}
			accessToken, err := s.claudeTokenProvider.GetAccessToken(ctx, account)
			if err != nil {
				return "", "", err
			}
			return accessToken, "wif", nil
		}
...
func (s *GatewayService) DoGrokNativeResponsesJSON(ctx context.Context, account *Account, body []byte) ([]byte, error) {
	...
	if resp.StatusCode >= 400 {
		...
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBytes, "grok upstream %d: %s", resp.StatusCode, sanitizeUpstreamErrorMessage(msg))
	}
