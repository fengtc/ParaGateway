...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	if tokenType != "apikey" && tokenType != wifAuthType {
		return nil, fmt.Errorf("anthropic API-key account passthrough requires apikey token or WIF token, got: %s", tokenType)
	}
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		upstreamReq, wireBody, err := s.buildUpstreamRequestAnthropicAPIKeyPassthroughWithTokenType(upstreamCtx, c, account, input.Body, token, tokenType)
		releaseUpstreamCtx()
		if err != nil {
			return nil, err
		}
	...
	if resp == nil || resp.Body == nil {
		return nil, newUpstreamTransportError(io.ErrUnexpectedEOF, "upstream request failed: empty response")
	}
...
func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthrough(
	...
	return s.buildUpstreamRequestAnthropicAPIKeyPassthroughWithTokenType(ctx, c, account, body, token, "apikey")
}

func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthroughWithTokenType(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	body []byte,
