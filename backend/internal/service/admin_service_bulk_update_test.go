...
type runtimeCircuitClearerForAdminTest struct {
	repo                   *accountRepoStubForBulkUpdate
	clearedIDs             []int64
	clearErr               error
	persistenceBeforeClear bool
}

func (s *runtimeCircuitClearerForAdminTest) BlockAccountScheduling(*Account, time.Time, string) {}
func (s *runtimeCircuitClearerForAdminTest) ClearAccountSchedulingBlock(int64)                  {}
func (s *runtimeCircuitClearerForAdminTest) ClearAccountRuntimeCircuit(_ context.Context, accountID int64) error {
	if s.repo != nil && (len(s.repo.bulkUpdateIDs) > 0 || len(s.repo.updatedAccounts) > 0) {
		s.persistenceBeforeClear = true
	}
...
func TestAdminServiceRuntimeBreakerChangeAdvancesGenerationBeforePersistence(t *testing.T) {
	t.Run("single update", func(t *testing.T) {
		account := &Account{ID: 41, Weight: 100, CircuitBreakerThreshold: 3, CircuitBreakerCooldownSeconds: 30}
		repo := &accountRepoStubForBulkUpdate{getByIDAccounts: map[int64]*Account{account.ID: account}}
		clearer := &runtimeCircuitClearerForAdminTest{repo: repo}
		svc := &adminServiceImpl{accountRepo: repo, runtimeBlocker: clearer}
		threshold := 8

		_, err := svc.UpdateAccount(context.Background(), account.ID, &UpdateAccountInput{CircuitBreakerThreshold: &threshold})
		require.NoError(t, err)
		require.Equal(t, []int64{account.ID}, clearer.clearedIDs)
		require.False(t, clearer.persistenceBeforeClear)
		require.Len(t, repo.updatedAccounts, 1)
	})

	t.Run("bulk partial field merges current cooldown", func(t *testing.T) {
		accounts := []*Account{
			{ID: 51, Weight: 100, CircuitBreakerThreshold: 3, CircuitBreakerCooldownSeconds: 30},
			{ID: 52, Weight: 100, CircuitBreakerThreshold: 4, CircuitBreakerCooldownSeconds: 45},
		}
		...
		require.NoError(t, err)
		require.Equal(t, []int64{51, 52}, clearer.clearedIDs)
		require.False(t, clearer.persistenceBeforeClear)
		require.Equal(t, []int64{51, 52}, repo.bulkUpdateIDs)
	})

	t.Run("clear failure prevents database write", func(t *testing.T) {
		account := &Account{ID: 61, Weight: 100, CircuitBreakerThreshold: 3, CircuitBreakerCooldownSeconds: 30}
		repo := &accountRepoStubForBulkUpdate{getByIDAccounts: map[int64]*Account{account.ID: account}}
		clearer := &runtimeCircuitClearerForAdminTest{repo: repo, clearErr: errors.New("redis unavailable")}
		svc := &adminServiceImpl{accountRepo: repo, runtimeBlocker: clearer}
		threshold := 7
