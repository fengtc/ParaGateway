...
const (
	defaultAccountWeight                        = 100
	defaultAccountRPMLimit                      = 0
	defaultAccountCircuitBreakerThreshold       = 0
	defaultAccountCircuitBreakerCooldownSeconds = 0
)

func normalizeAccountRuntimePolicy(input *CreateAccountInput) error {
	if input.Weight == 0 {
		input.Weight = defaultAccountWeight
	}
	return validateAccountRuntimePolicy(input.Weight, input.RPMLimit, input.CircuitBreakerThreshold, input.CircuitBreakerCooldownSeconds)
}

func validateAccountRuntimePolicy(weight, rpmLimit, threshold, cooldownSeconds int) error {
	if weight <= 0 || weight > 1_000_000 {
		return errors.New("weight must be between 1 and 1000000")
	}
	if rpmLimit < 0 || rpmLimit > 1_000_000 {
		return errors.New("rpm_limit must be between 0 and 1000000")
	}
	if threshold < 0 || threshold > 1000 {
		return errors.New("circuit_breaker_threshold must be between 0 and 1000")
	}
	if cooldownSeconds < 0 || cooldownSeconds > 86400 {
		return errors.New("circuit_breaker_cooldown_seconds must be between 0 and 86400")
	}
...
func buildAccountForCreate(input *CreateAccountInput, accountExtra map[string]any) (*Account, error) {
	if err := normalizeAccountRuntimePolicy(input); err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if account.IsWIF() && s.wifTokenProvider == nil {
		return nil, infraerrors.New(http.StatusInternalServerError, "WIF_PROVIDER_UNAVAILABLE", "WIF token provider is not configured")
	}
	if s.wifTokenProvider != nil {
		if err := s.wifTokenProvider.PrepareAccountForPersistence(account); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Weight != nil || input.RPMLimit != nil || input.CircuitBreakerThreshold != nil || input.CircuitBreakerCooldownSeconds != nil {
		weight := account.EffectiveWeight()
		rpmLimit := account.EffectiveRPMLimit()
		threshold := account.EffectiveCircuitBreakerThreshold()
		cooldownSeconds := int(account.EffectiveCircuitBreakerCooldown() / time.Second)
		if input.Weight != nil {
			weight = *input.Weight
		}
		...
		if input.CircuitBreakerThreshold != nil {
			threshold = *input.CircuitBreakerThreshold
		}
		if input.CircuitBreakerCooldownSeconds != nil {
			cooldownSeconds = *input.CircuitBreakerCooldownSeconds
		}
		if err := validateAccountRuntimePolicy(weight, rpmLimit, threshold, cooldownSeconds); err != nil {
			return nil, err
		}
		account.Weight = weight
		account.RPMLimit = rpmLimit
		account.CircuitBreakerThreshold = threshold
		account.CircuitBreakerCooldownSeconds = cooldownSeconds
	}
	if input.RateMultiplier != nil {
		if *input.RateMultiplier < 0 {
			return nil, errors.New("rate_multiplier must be >= 0")
	...
	if account.IsWIF() && (!wasWIF || previousWIFCredentialBoundary != wifCredentialBoundary(account)) {
		// Never reinterpret the account's existing provider API key as a WIF
		// client secret or reuse an undisclosed secret after its issuer boundary
		// changes. The subject-token URL is administrator supplied, so either case
		// would turn an incomplete update into credential disclosure.
		incomingSecret, explicitlyProvided := input.Credentials[wifClientSecretCredentialKey]
		secret, isString := incomingSecret.(string)
		if !explicitlyProvided || !isString || strings.TrimSpace(secret) == "" {
			if !wasWIF {
				return nil, infraerrors.BadRequest("WIF_ENABLE_REQUIRES_CLIENT_SECRET", "a new WIF client secret is required when enabling WIF authentication")
			}
			return nil, infraerrors.BadRequest("WIF_AUTH_BOUNDARY_CHANGE_REQUIRES_CLIENT_SECRET", "a new WIF client secret is required when the WIF authentication boundary changes")
		}
	}
	if s.wifTokenProvider != nil {
		if err := s.wifTokenProvider.PrepareAccountForPersistence(account); err != nil {
	...
	// 熔断策略属于带请求代际的运行时状态。策略一旦发生变化，必须先推进
	// Redis 代际并清理旧失败计数，再写数据库；否则旧的在途请求可能在新策略
	// 生效后按旧 admission token 回写并错误开路。显式重复关闭也幂等推进，
	// 这样管理请求在响应丢失后重试仍能可靠清掉遗留状态。
	breakerPolicyTouched := input.CircuitBreakerThreshold != nil || input.CircuitBreakerCooldownSeconds != nil
	breakerPolicyChanged := account.EffectiveCircuitBreakerThreshold() != previousCircuitBreakerThreshold ||
		int(account.EffectiveCircuitBreakerCooldown()/time.Second) != previousCircuitBreakerCooldownSeconds
	breakerPolicyDisabled := account.EffectiveCircuitBreakerThreshold() == 0 && account.EffectiveCircuitBreakerCooldown() == 0
	if breakerPolicyTouched && (breakerPolicyChanged || breakerPolicyDisabled) {
		if clearer, ok := s.runtimeBlocker.(AccountRuntimeCircuitClearer); ok {
			if err := clearer.ClearAccountRuntimeCircuit(ctx, account.ID); err != nil {
				return nil, fmt.Errorf("advance account circuit policy generation: %w", err)
			}
		}
	}
...
func (s *adminServiceImpl) UpdateAccountExtra(ctx context.Context, id int64, updates map[string]any) error {
	if containsWIFManagedExtraKey(updates) {
		return infraerrors.BadRequest("WIF_FULL_ACCOUNT_UPDATE_REQUIRED", "WIF configuration must be changed through the full account update endpoint")
	}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	if containsWIFManagedExtraKey(input.Extra) {
		return nil, infraerrors.BadRequest("WIF_BULK_UPDATE_UNSUPPORTED", "WIF configuration cannot be changed with bulk update")
	}
	...
	if len(input.Credentials) > 0 {
		for _, acc := range cachedTargets {
			...
			if acc != nil && acc.IsWIF() {
				return nil, infraerrors.BadRequest("WIF_BULK_CREDENTIAL_UPDATE_UNSUPPORTED", "WIF credentials must be changed through the single-account update endpoint")
			}
		}
	}
	...
	bulkCircuitAdvanceIDs := make([]int64, 0)
	if input.Weight != nil || input.RPMLimit != nil || input.CircuitBreakerThreshold != nil || input.CircuitBreakerCooldownSeconds != nil {
		targetsByID := make(map[int64]*Account, len(cachedTargets))
		...
		for _, accountID := range input.AccountIDs {
			...
			weight := target.EffectiveWeight()
			rpmLimit := target.EffectiveRPMLimit()
			threshold := target.EffectiveCircuitBreakerThreshold()
			cooldownSeconds := int(target.EffectiveCircuitBreakerCooldown() / time.Second)
			if input.Weight != nil {
				weight = *input.Weight
			}
			if input.RPMLimit != nil {
			...
			breakerPolicyTouched := input.CircuitBreakerThreshold != nil || input.CircuitBreakerCooldownSeconds != nil
			breakerPolicyChanged := threshold != target.EffectiveCircuitBreakerThreshold() ||
				cooldownSeconds != int(target.EffectiveCircuitBreakerCooldown()/time.Second)
			breakerPolicyDisabled := threshold == 0 && cooldownSeconds == 0
			if breakerPolicyTouched && (breakerPolicyChanged || breakerPolicyDisabled) {
				bulkCircuitAdvanceIDs = append(bulkCircuitAdvanceIDs, accountID)
			}
		}
		repoUpdates.Weight = input.Weight
		repoUpdates.RPMLimit = input.RPMLimit
		repoUpdates.CircuitBreakerThreshold = input.CircuitBreakerThreshold
		repoUpdates.CircuitBreakerCooldownSeconds = input.CircuitBreakerCooldownSeconds
	}
	if input.RateMultiplier != nil {
	...
	// 与单账号更新相同，先推进所有受影响账号的运行时代际，全部成功后才允许
	// 数据库批量写入。部分账号已推进而后续账号清理失败是安全的：下一次选号
	// 会建立新代际，且数据库策略尚未改变。
	if clearer, ok := s.runtimeBlocker.(AccountRuntimeCircuitClearer); ok {
		for _, accountID := range bulkCircuitAdvanceIDs {
			if err := clearer.ClearAccountRuntimeCircuit(ctx, accountID); err != nil {
				return nil, fmt.Errorf("advance account circuit policy generation %d: %w", accountID, err)
			}
		}
	}

	// Run bulk update for column/jsonb fields after runtime-policy fencing.
