...
func TestContentModerationConfigView_EmptyCollectionsMarshalAsArrays(t *testing.T) {
	settingRepo := &contentModerationTestSettingRepo{values: map[string]string{}}
	svc := NewContentModerationService(settingRepo, nil, nil, nil, nil, nil, nil, nil)

	view, err := svc.GetConfig(context.Background())
	require.NoError(t, err)
	require.NotNil(t, view.GroupIDs)
	require.NotNil(t, view.BlockedKeywords)
	require.NotNil(t, view.ModelFilter.Models)

	raw, err := json.Marshal(view)
	require.NoError(t, err)
	require.NotContains(t, string(raw), `"group_ids":null`)
	require.NotContains(t, string(raw), `"blocked_keywords":null`)
	require.NotContains(t, string(raw), `"models":null`)
}
