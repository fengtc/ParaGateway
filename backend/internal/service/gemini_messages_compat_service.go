...
func (s *GeminiMessagesCompatService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		if err != nil {
			...
			_ = s.writeClaudeError(c, http.StatusBadGateway, "upstream_error", "Upstream request failed after retries: "+safeErr)
			return nil, newUpstreamTransportError(err, safeErr)
		}
...
func (s *GeminiMessagesCompatService) ForwardNative(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		if err != nil {
			...
			setOpsUpstreamError(c, 0, safeErr, "")
			_ = s.writeGoogleError(c, http.StatusBadGateway, "Upstream request failed after retries: "+safeErr)
			return nil, newUpstreamTransportError(err, safeErr)
		}
	...
	if resp.StatusCode >= 400 {
		...
		if upstreamMsg == "" {
			return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "gemini upstream error: %d", resp.StatusCode)
		}
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "gemini upstream error: %d message=%s", resp.StatusCode, upstreamMsg)
	}
...
func (s *GeminiMessagesCompatService) writeGeminiMappedError(c *gin.Context, account *Account, upstreamStatus int, upstreamRequestID string, body []byte) error {
	...
	if status, errType, errMsg, matched := applyErrorPassthroughRule(
		...
		if upstreamMsg == "" {
			return newUpstreamHTTPStatusError(upstreamStatus, body, "upstream error: %d (passthrough rule matched)", upstreamStatus)
		}
		return newUpstreamHTTPStatusError(upstreamStatus, body, "upstream error: %d (passthrough rule matched) message=%s", upstreamStatus, upstreamMsg)
	}
	...
	if upstreamMsg == "" {
		return newUpstreamHTTPStatusError(upstreamStatus, body, "upstream error: %d", upstreamStatus)
	}
	return newUpstreamHTTPStatusError(upstreamStatus, body, "upstream error: %d message=%s", upstreamStatus, upstreamMsg)
}
