...
func NormalizeUsageWorkAttribution(value UsageWorkAttribution) UsageWorkAttribution {
	...
	if len(value.ClassifierVersion) > 64 {
		value.ClassifierVersion = value.ClassifierVersion[:64]
	}
	value.Confidence = math.Max(0, math.Min(1, value.Confidence))
	if value.ClassificationSource != "local_rule" && value.ClassificationSource != "import" {
		value.ClassificationSource = "unclassified"
	}
	if _, ok := validWorkCategories[value.Category]; !ok {
		value.Category = WorkCategoryUnclassified
	}
	switch value.Category {
	case WorkCategoryUnclassified:
		value.WorkRelated = WorkRelatedUncertain
	case WorkCategoryNonWork:
		value.WorkRelated = WorkRelatedNonWork
	default:
		value.WorkRelated = WorkRelatedWork
	}
	if value.ClassificationSource == "unclassified" {
		value.WorkRelated = WorkRelatedUncertain
		value.Category = WorkCategoryUnclassified
	}
	return value
...
func ApplyUsageWorkAttribution(ctx context.Context, usageLog *UsageLog) {
	...
	if value, ok := UsageWorkAttributionFromContext(ctx); ok {
		value = NormalizeUsageWorkAttribution(value)
		usageLog.WorkAttribution = &value
	}
}
