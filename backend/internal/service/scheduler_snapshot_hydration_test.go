...
func TestOpenAIWIFMetadataSelectionHydratesCompleteAccount(t *testing.T) {
	cache := &snapshotHydrationCache{
		snapshot: []*Account{{
			ID:                            81,
			Platform:                      PlatformOpenAI,
	...
	if selection == nil || selection.Account == nil {
		t.Fatal("expected selected WIF account")
	}
	hydrated := selection.Account
	if !hydrated.IsWIF() {
		t.Fatal("expected full hydrated account to retain WIF identity")
	}
	if got := hydrated.GetCredential(wifClientSecretCiphertextKey); got != "encrypted-wif-secret" {
		t.Fatalf("expected hydrated WIF ciphertext, got %q", got)
	}
	configuration, enabled, err := WIFConfigurationFromAccount(hydrated)
	if err != nil || !enabled {
		t.Fatalf("expected valid hydrated WIF configuration: enabled=%v err=%v", enabled, err)
	}
	if configuration.ClientID != "client-81" {
		t.Fatalf("expected hydrated WIF client id, got %q", configuration.ClientID)
	}
}
...
func TestGatewayNewAcquiredSelectionResult_ReleasesSlotWhenHydrationFails(t *testing.T) {
	...
	schedulerSnapshot := NewSchedulerSnapshotService(cache, nil, stubOpenAIAccountRepo{}, nil, nil)
	svc := &GatewayService{schedulerSnapshot: schedulerSnapshot}
	releaseCalls := 0
