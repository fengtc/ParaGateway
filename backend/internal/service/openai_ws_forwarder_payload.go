...
func (s *OpenAIGatewayService) buildOpenAIResponsesWSURL(account *Account) (string, error) {
	...
	if account.IsGitHubCopilot() {
		return "", copilotUnsupportedEndpointError("Responses WebSocket")
	}
...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	if account != nil && account.IsGitHubCopilot() {
		return nil, openAIWSSessionHeaderResolution{}, copilotUnsupportedEndpointError("Responses WebSocket")
	}
