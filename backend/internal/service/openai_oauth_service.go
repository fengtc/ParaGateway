...
func NewOpenAIOAuthService(proxyRepo ProxyRepository, oauthClient OpenAIOAuthClient) *OpenAIOAuthService {
	return &OpenAIOAuthService{
		...
		copilotFlows:      make(map[string]*copilotOAuthFlow),
		copilotHTTPClient: &http.Client{Timeout: copilotRequestTimeout},
		copilotEndpoints:  defaultCopilotOAuthEndpoints(),
	}
}
...
func (s *OpenAIOAuthService) RefreshAccountToken(ctx context.Context, account *Account) (*OpenAITokenInfo, error) {
	...
	if account.IsGitHubCopilot() {
		githubToken := account.GetGitHubAccessToken()
		...
		return &OpenAITokenInfo{
			...
			OAuthProfile:      CopilotOAuthProfile,
...
func (s *OpenAIOAuthService) BuildAccountCredentials(tokenInfo *OpenAITokenInfo) map[string]any {
	...
	if tokenInfo.RefreshAt > 0 {
		creds["refresh_at"] = time.Unix(tokenInfo.RefreshAt, 0).Format(time.RFC3339)
	}
	...
	if strings.TrimSpace(tokenInfo.OAuthProfile) != "" {
		creds["oauth_profile"] = strings.TrimSpace(tokenInfo.OAuthProfile)
	}
	if strings.TrimSpace(tokenInfo.GitHubAccessToken) != "" {
		creds["github_access_token"] = tokenInfo.GitHubAccessToken
	}
	if strings.TrimSpace(tokenInfo.GitHubLogin) != "" {
		creds["github_login"] = strings.TrimSpace(tokenInfo.GitHubLogin)
	}
	if strings.TrimSpace(tokenInfo.GitHubUserID) != "" {
		creds["github_user_id"] = strings.TrimSpace(tokenInfo.GitHubUserID)
	}
	if strings.TrimSpace(tokenInfo.BaseURL) != "" {
		creds["base_url"] = strings.TrimRight(strings.TrimSpace(tokenInfo.BaseURL), "/")
	}
