...
func TestRelay_TerminalClientWriteFailureDoesNotReportHealthyTurn(t *testing.T) {
	...
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{{
		msgType: coderws.MessageText,
		payload: []byte(`{"type":"response.completed","response":{"id":"resp_write_fail","usage":{"input_tokens":2,"output_tokens":1}}}`),
	}}, true)
	var completed atomic.Int32

	result, relayExit := Relay(context.Background(), clientConn, upstreamConn,
		[]byte(`{"type":"response.create","model":"gpt-4o","input":[]}`),
		RelayOptions{OnTurnComplete: func(RelayTurnResult) { completed.Add(1) }},
	)

	require.NotNil(t, relayExit)
	require.Equal(t, "write_client", relayExit.Stage)
	require.Equal(t, int32(0), completed.Load(), "terminal turn must not report healthy before downstream write succeeds")
