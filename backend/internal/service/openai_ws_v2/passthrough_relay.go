...
func Relay(
	...
	result.ClientDisconnect = firstExit.stage == "read_client" && firstExit.graceful
...
func runUpstreamToClient(
	...
	for {
		...
		if dropDownstreamWrites != nil && dropDownstreamWrites.Load() {
			emitTurnComplete(onTurnComplete, state, observedEvent, dropDownstreamWrites)
		...
		// Terminal success must only be reported after the downstream write has
		// succeeded. Reporting before WriteFrame would briefly clear account
		// breaker state even when the client had already disconnected.
		emitTurnComplete(onTurnComplete, state, observedEvent, nil)
...
func emitTurnComplete(
	...
	if len(disconnectFlags) > 0 && disconnectFlags[0] != nil {
		clientDisconnect = disconnectFlags[0].Load()
	}
