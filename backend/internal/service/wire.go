...
// ProvideWIFTokenProvider creates the shared memory-only WIF access-token cache.
// A fixed key is mandatory for new WIF accounts so ciphertext remains usable
// after process restarts.
func ProvideWIFTokenProvider(encryptor SecretEncryptor, cfg *config.Config) *WIFTokenProvider {
	provider := NewWIFTokenProvider()
	persistent := cfg != nil && cfg.Totp.EncryptionKeyConfigured
	provider.SetSecretEncryptor(encryptor, persistent)
	return provider
}
...
func ProvideClaudeTokenProvider(
	...
	p.SetWIFTokenProvider(wifTokenProvider)
...
func ProvideOpenAIGatewayService(
	...
	service.SetAccountRuntimePolicyCache(rpmCache)
	return service
}
...
var ProviderSet = wire.NewSet(
	...
	NewAccountService,
	NewUpstreamAccountService,
	NewProxyService,
	NewRedeemService,
	NewPromoService,
	NewUsageService,
	NewWorkDistributionService,
	NewDashboardService,
