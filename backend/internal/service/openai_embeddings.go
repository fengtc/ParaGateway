...
func (s *OpenAIGatewayService) ForwardEmbeddings(
	...
	if account != nil && account.IsGitHubCopilot() {
		return nil, copilotUnsupportedEndpointError("Embeddings")
	}
	...
	if resp.StatusCode >= 400 {
		...
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream returned status %d", resp.StatusCode)
	}
