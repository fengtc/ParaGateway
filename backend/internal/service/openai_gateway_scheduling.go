...
func (s *OpenAIGatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	...
	for {
		account, err := s.selectAccountForModelWithExclusions(s.withOpenAIQuotaAutoPauseContext(ctx), groupID, PlatformOpenAI, sessionHash, requestedModel, localExcluded, false, 0, "", false)
		if err != nil {
			return nil, err
		}
		gate, err := acquireAccountRuntimePolicy(ctx, s.accountRuntimePolicy, account)
		if err != nil {
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool, openAISelectionFilterStats) {
	...
	return selectOpenAIHardLayerByWeightOrFirst(eligible, compactTiers, requireCompact, rateOrder), compactBlocked, filterStats
}

// selectOpenAIHardLayerByWeightOrFirst keeps compact support, upstream rate and
// priority as hard scheduling layers. An explicitly different account weight is
// applied only inside the current best layer; when all weights are equal the
// already sorted first account is returned, preserving the legacy LRU order.
func selectOpenAIHardLayerByWeightOrFirst(
	ordered []*Account,
	...
	first := ordered[0]
	pool := make([]*Account, 0, len(ordered))
	for _, candidate := range ordered {
		if candidate == nil || candidate.Priority != first.Priority {
			continue
		}
		if requireCompact && compactTiers[candidate.ID] != compactTiers[first.ID] {
			continue
		}
		if rateOrder.compare(first, candidate) != 0 {
			continue
		}
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	localExcluded := cloneAccountExclusions(excludedIDs)
	for {
		selection, err := s.selectAccountWithLoadAwarenessOnce(ctx, groupID, sessionHash, requestedModel, localExcluded)
		if err != nil {
			return nil, err
		}
