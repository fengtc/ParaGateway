...
// 特点：不记录使用量、仅支持非流式响应
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (err error) {
	if parsed == nil {
		s.countTokensError(c, http.StatusBadRequest, "invalid_request_error", "Request body is empty")
		return fmt.Errorf("parse request: empty request")
	}
	defer func() { s.RecordAccountRuntimeOutcome(ctx, account, AccountRuntimeOutcomeFromContext(ctx, err)) }()
	...
	if account != nil && account.IsBedrock() {
		...
		return fmt.Errorf("count_tokens endpoint is not supported for Bedrock")
	}
	...
	if resp.StatusCode >= 400 {
		...
		if upstreamMsg == "" {
			return newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream error: %d", resp.StatusCode)
		}
		return newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream error: %d message=%s", resp.StatusCode, upstreamMsg)
	}
...
func (s *GatewayService) forwardCountTokensAnthropicAPIKeyPassthrough(ctx context.Context, c *gin.Context, account *Account, body []byte) error {
	...
	if tokenType != "apikey" && tokenType != wifAuthType {
		s.countTokensError(c, http.StatusBadGateway, "upstream_error", "Invalid account token type")
		return fmt.Errorf("anthropic API-key account passthrough requires apikey token or WIF token, got: %s", tokenType)
	}

	upstreamReq, err := s.buildCountTokensRequestAnthropicAPIKeyPassthroughWithTokenType(ctx, c, account, body, token, tokenType)
	if err != nil {
		s.countTokensError(c, http.StatusInternalServerError, "api_error", "Failed to build request")
		return err
	}
...
func (s *GatewayService) buildCountTokensRequestAnthropicAPIKeyPassthrough(
	...
	return s.buildCountTokensRequestAnthropicAPIKeyPassthroughWithTokenType(ctx, c, account, body, token, "apikey")
}

func (s *GatewayService) buildCountTokensRequestAnthropicAPIKeyPassthroughWithTokenType(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	body []byte,
	...
	if isAnthropicBearerTokenType(tokenType) {
