...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	if resp.StatusCode >= 400 {
		...
		if account.IsGitHubCopilot() && isCopilotUnsupportedAPIForModel(resp.StatusCode, respBody) {
			logger.L().Info("copilot chat_completions: model requires Responses; retrying once",
				zap.Int64("account_id", account.ID),
				zap.String("upstream_model", upstreamModel),
			)
			return s.forwardCopilotChatCompletionsViaResponses(ctx, c, account, body, defaultMappedModel)
		}
		if account.Platform == PlatformGrok {
			kind := "http_error"
...
func (s *OpenAIGatewayService) streamRawChatCompletions(
	...
	for scanner.Scan() {
		...
		// Copilot uses dotted Claude IDs upstream (for example
		// claude-sonnet-4.5), while the public gateway catalog uses the
		// hyphenated spelling. Rewrite only the outgoing JSON data line; usage
		// and response observation above continue to inspect the upstream payload.
		if account.IsGitHubCopilot() {
			line = rewriteCopilotSSEModelLine(line)
		}
		writeLine(line)
		if line == "" {
	...
	if err := scanner.Err(); err != nil {
		...
		if !clientOutputStarted && !clientDisconnected {
			message := "OpenAI chat completions stream disconnected before output"
			if errText := strings.TrimSpace(err.Error()); errText != "" {
				message += ": " + errText
			}
			return nil, s.newOpenAIStreamFailoverError(c, account, false, requestID, nil, message, resp.Header)
		}
		return resultWithUsage(), newUpstreamTransportError(err, "OpenAI chat completions stream disconnected before completion")
...
// rewriteCopilotSSEModelLine converts the top-level model field in one SSE
// data line without disturbing event/blank/comment lines or the [DONE] marker.
func rewriteCopilotSSEModelLine(line string) string {
	...
	clientModel := copilotModelForClient(model)
	if clientModel == model {
		return line
	}
	rewritten, err := sjson.SetBytes([]byte(payload), "model", clientModel)
	if err != nil {
		return line
	}
	dataIndex := strings.Index(strings.ToLower(line), "data:")
	if dataIndex < 0 {
		return line
	}
	prefixEnd := dataIndex + len("data:")
...
func (s *OpenAIGatewayService) bufferRawChatCompletions(
	...
	if account.IsGitHubCopilot() && responseModel != "" {
		clientModel := copilotModelForClient(responseModel)
		if clientModel != responseModel {
			if rewritten, rewriteErr := sjson.SetBytes(respBody, "model", clientModel); rewriteErr == nil {
				respBody = rewritten
			}
		}
	}
