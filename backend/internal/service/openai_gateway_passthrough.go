...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if account != nil && account.IsGitHubCopilot() {
		return nil, copilotUnsupportedEndpointError("Responses 透传")
	}
...
// newOpenAIStreamFailureStatusError preserves the semantic status carried by a
// response.failed event (which itself arrives over HTTP 200) for runtime account
// health classification. Policy/content rejections are request-scoped and must
// never trip the account circuit breaker.
func newOpenAIStreamFailureStatusError(payload []byte, message string) error {
	...
	if safeMessage == "" {
		safeMessage = "OpenAI upstream response failed"
	}
	policyText := strings.ToLower(strings.TrimSpace(string(body) + " " + safeMessage))
	if hit, _, _ := detectOpenAICyberPolicy(payload); hit ||
		strings.Contains(policyText, "content_policy") ||
		strings.Contains(policyText, "content policy") ||
		strings.Contains(policyText, "safety_policy") ||
		strings.Contains(policyText, "safety policy") ||
		strings.Contains(policyText, "moderation") {
		return newRequestScopedUpstreamHTTPStatusError(status, body, "upstream response rejected by content policy: %s", safeMessage)
	}
	return newUpstreamHTTPStatusError(status, body, "upstream response failed: %s", safeMessage)
}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
			...
			if eventType == "response.failed" {
				...
				if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
					if status, errType, errMsg, matched := applyOpenAIStreamFailedErrorPassthroughRule(c, account.Platform, dataBytes, failedMessage); matched {
						...
						return resultWithUsage(), newOpenAIStreamFailureStatusError(dataBytes, firstNonEmpty(errMsg, failedMessage))
					}
	...
	if err := documentScanner.Err(); err != nil {
		...
		if sawFailedEvent {
			return resultWithUsage(), newOpenAIStreamFailureStatusError(failedPayload, failedMessage)
		}
