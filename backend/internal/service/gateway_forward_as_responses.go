...
func (s *GatewayService) ForwardAsResponses(
	...
	if resp.StatusCode >= 400 {
		...
		return nil, newUpstreamHTTPStatusError(resp.StatusCode, respBody, "upstream error: %d %s", resp.StatusCode, upstreamMsg)
	}
...
func (s *GatewayService) handleResponsesBufferedStreamingResponse(
	...
	if finalResp == nil {
		...
		return nil, newUpstreamTransportError(io.ErrUnexpectedEOF, "upstream stream ended without response")
	}
