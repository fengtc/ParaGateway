...
func (s *OpenAIGatewayService) FetchCodexModelsManifest(ctx context.Context, account *Account, clientVersion, ifNoneMatch string) (*CodexModelsManifest, error) {
	...
	if credAccount.IsGitHubCopilot() {
		return buildCopilotCodexModelsManifest(ifNoneMatch)
	}
...
func buildCopilotCodexModelsManifest(ifNoneMatch string) (*CodexModelsManifest, error) {
	...
	for _, model := range models {
		if model = strings.TrimSpace(model); model != "" {
			entries = append(entries, modelEntry{Slug: model})
		}
	}
	body, err := json.Marshal(map[string][]modelEntry{"models": entries})
	if err != nil {
		return nil, infraerrors.Newf(http.StatusInternalServerError, "COPILOT_MODELS_MANIFEST_FAILED", "build Copilot models manifest: %v", err)
	}
