...
func (s *OpenAIGatewayService) ForwardCountTokensAsAnthropic(
	...
	if account.IsGitHubCopilot() {
		err := copilotUnsupportedEndpointError("Messages count_tokens")
		writeAnthropicCountTokensError(c, infraerrors.Code(err), "not_supported_error", infraerrors.Message(err))
		return err
	}
	...
	if err != nil {
		...
		return newUpstreamTransportError(err, safeErr)
	}
	defer func() { _ = resp.Body.Close() }()
	...
	if resp.StatusCode >= 400 {
		...
		if isOpenAIInputTokensUnsupported(resp.StatusCode, respBody) {
			...
			return newUpstreamHTTPStatusError(http.StatusNotFound, respBody, "input_tokens endpoint is not supported by upstream")
		}
		...
		if upstreamMsg == "" {
			return newUpstreamHTTPStatusError(resp.StatusCode, respBody, "input_tokens upstream error: %d", resp.StatusCode)
		}
		return newUpstreamHTTPStatusError(resp.StatusCode, respBody, "input_tokens upstream error: %d message=%s", resp.StatusCode, upstreamMsg)
	}
