...
func (s *OpenAIGatewayService) handleChatStreamingResponse(
	...
	processDataLine := func(payload string) bool {
		...
		if strings.TrimSpace(event.Type) == "response.failed" {
			...
			streamNonFailoverErr = newOpenAIStreamFailureStatusError(payloadBytes, message)
			return true
		}
	...
	missingTerminalErr := func() (*OpenAIForwardResult, error) {
		return resultWithUsage(), newUpstreamTransportError(io.ErrUnexpectedEOF, "stream usage incomplete: missing terminal event")
	}
