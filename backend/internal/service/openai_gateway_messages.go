...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.IsGitHubCopilot() {
		return s.forwardCopilotAnthropic(ctx, c, account, body, defaultMappedModel)
	}
...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	if finalResponse == nil {
		...
		return nil, newUpstreamTransportError(io.ErrUnexpectedEOF, "upstream stream ended without terminal event")
	}
	...
	if strings.TrimSpace(finalResponse.Status) == "failed" {
		...
		if hit, code, msg := detectOpenAICyberPolicy(payload); hit {
			...
			return nil, newOpenAIStreamFailureStatusError(payload, msg)
		}
		...
		if status, errType, errMsg, matched := applyOpenAIStreamFailedErrorPassthroughRule(
			...
			return nil, newOpenAIStreamFailureStatusError(payload, errMsg)
		}
		writeAnthropicError(c, http.StatusBadGateway, "api_error", message)
		return nil, newOpenAIStreamFailureStatusError(payload, message)
	}
...
func (s *OpenAIGatewayService) handleAnthropicStreamingResponse(
	...
	processDataLine := func(payload string) bool {
		...
		if isTerminalEvent {
			...
			if eventType == "response.failed" || isBareErrorEvent {
				...
				streamNonFailoverErr = newOpenAIStreamFailureStatusError(payloadBytes, errMsg)
				return true
			}
		}
	...
	missingTerminalErr := func() (*OpenAIForwardResult, error) {
		...
		return result, newUpstreamTransportError(io.ErrUnexpectedEOF, "stream usage incomplete: missing terminal event")
	}
