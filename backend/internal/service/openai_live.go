...
func (s *OpenAIGatewayService) CreateLiveCall(
	...
	for attempt := 0; attempt <= 3; attempt++ {
		...
		s.ReportOpenAIAccountSelectionOutcome(selection, scheduleModel, AccountRuntimeOutcomeFromContext(ctx, createErr), nil)
		if createErr != nil {
...
func (s *OpenAIGatewayService) createUpstreamLiveCall(
	...
	if account.IsGitHubCopilot() {
		return nil, copilotUnsupportedEndpointError("Live")
	}
	// Live is a ChatGPT-only upstream. Guard again at the transport boundary so
	// a direct/internal call can never send an API-key or WIF bearer token to
	// chatgpt.com even if scheduler capability filtering is bypassed.
	if !account.SupportsOpenAIEndpointCapability(OpenAIEndpointCapabilityLive) {
		return nil, ErrLiveUnavailable
	}
...
func (s *OpenAIGatewayService) liveSidebandHeaders(
	...
	// A persisted/corrupt live record must not bypass the create-call guard and
	// leak a non-ChatGPT credential to the sideband WebSocket.
...
func (s *OpenAIGatewayService) finalizeLiveCall(record *LiveCallRecord) {
	...
	if record.WorkAttribution != nil {
		attribution := NormalizeUsageWorkAttribution(*record.WorkAttribution)
