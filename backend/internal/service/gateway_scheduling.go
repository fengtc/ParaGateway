...
func (s *GatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	localExcluded := cloneAccountExclusions(excludedIDs)
	for {
		account, err := s.selectAccountForModelWithExclusionsOnce(ctx, groupID, sessionHash, requestedModel, localExcluded)
		if err != nil {
			return nil, err
		}
		gate, err := acquireAccountRuntimePolicy(ctx, s.accountRuntimePolicyStore(), account)
		if err != nil {
			return nil, fmt.Errorf("acquire account runtime policy: %w", err)
		}
		if gate.Allowed {
			return accountWithRuntimePolicyGeneration(account, gate.Generation), nil
		}
		localExcluded[account.ID] = struct{}{}
		if len(localExcluded) > 10000 {
			return nil, accountRuntimeGateError(gate)
		}
	}
}
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	localExcluded := cloneAccountExclusions(excludedIDs)
	for {
		selection, err := s.selectAccountWithLoadAwarenessOnce(ctx, groupID, sessionHash, requestedModel, localExcluded, metadataUserID, sub2apiUserID)
		if err != nil {
			return nil, err
		}
		...
		gate, err := acquireAccountRuntimePolicy(ctx, s.accountRuntimePolicyStore(), selection.Account)
		if err != nil {
		...
		if selection.PreserveStickyBinding {
			ctx = WithPreserveCopilotStickyBinding(ctx)
		}
		localExcluded[selection.Account.ID] = struct{}{}
	}
}
...
func (s *GatewayService) selectAccountWithLoadAwarenessOnce(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	// A busy Copilot sticky account is a soft affinity: overflow requests may
	// use another account, but must not migrate the existing session binding.
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		...
		for {
			...
			account, err := s.selectAccountForModelWithExclusionsOnce(ctx, groupID, selectionSessionHash, requestedModel, localExcluded)
			if err != nil {
	...
	if len(routingAccountIDs) > 0 && s.concurrencyService != nil {
		...
		if len(routingCandidates) > 0 {
			...
			// 3. 按负载感知排序。Copilot 逃逸期间不再尝试原粘性账号。
			...
			// Cached load may be stale. Recheck once before putting a Copilot
			// overflow request into a wait queue.
			if preserveStickyBinding {
				if freshLoadMap, freshErr := s.concurrencyService.GetAccountsLoadBatchFresh(ctx, routingLoads); freshErr == nil {
					routingAvailable = buildRoutingAvailable(freshLoadMap)
					selection, acquireErr = tryAcquireRouted(routingAvailable)
					if acquireErr != nil {
						return nil, acquireErr
					}
	...
	tryAcquireFromLoadMap := func(loadMap map[int64]*AccountLoadInfo) (*AccountSelectionResult, error) {
		...
		// Worker 对齐：priority 是硬层级。只有同一硬层内显式配置了不同
		// weight 时，才按账号独立 weight 抽取；默认 weight 全相同时完整保留
		// 既有的最低负载 + LRU/OAuth 偏好，避免升级后改变存量流量分配。
		for len(available) > 0 {
			// 1. 取优先级最小的集合
			...
			// 3. 显式权重有差异时权重优先；否则走原最低负载 + LRU。
			selected := selectGenericLoadAwareCandidate(priorityCandidates, preferOAuth)
			if selected == nil {
				break
			}
...
func shouldEscapeCopilotStickyOnConcurrencyFull(account *Account, result *AcquireResult, acquireErr error) bool {
	return account != nil && account.IsGitHubCopilot() && isAccountConcurrencyFull(result, acquireErr)
}
...
// Other failover reasons retain the normal sticky rebinding behavior.
...
func (s *GatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	if s.schedulerSnapshot != nil {
		...
		if err == nil {
			...
			accounts, err = s.hydrateGatewayCopilotBillingAccountsForScheduling(ctx, accounts)
			if err != nil {
	...
	if useMixed {
		...
		prefetchCopilotBillingGuards(ctx, filtered)
...
func isMixedSchedulingPlatformAllowed(account *Account, nativePlatform string) bool {
	...
	if nativePlatform == PlatformAnthropic && account.IsGitHubCopilot() {
		return true
	}
...
// isAccountSchedulableForQuota checks both the Copilot monthly Billing guard
// and quota_limit for API key / Bedrock accounts.
func (s *GatewayService) isAccountSchedulableForQuota(ctx context.Context, account *Account) bool {
	...
	if skip, used, limit := shouldSkipCopilotAccountForBilling(ctx, account); skip {
		slog.Debug("gateway_copilot_billing_guard_skip",
			"account_id", account.ID,
			"used_credits", used,
			"stop_limit", limit,
		)
...
// hydrateGatewayCopilotBillingAccountsForScheduling replaces scheduler
// metadata with request-local full account copies before the Billing guard
// runs. Scheduler snapshots intentionally omit PATs and other secrets.
func (s *GatewayService) hydrateGatewayCopilotBillingAccountsForScheduling(ctx context.Context, accounts []Account) ([]Account, error) {
	...
	for i := range accounts {
		...
		if hydrated[i] == nil {
			slog.Warn("gateway_copilot_billing_account_hydration_failed",
...
func selectByLRU(accounts []accountWithLoad, preferOAuth bool) *accountWithLoad {
	...
	weightsDiffer := false
	firstWeight := accounts[candidateIdxs[0]].account.EffectiveWeight()
	for _, idx := range candidateIdxs[1:] {
		if accounts[idx].account.EffectiveWeight() != firstWeight {
			weightsDiffer = true
			break
		}
	}
	...
	// 5. 显式配置不同 weight 时按独立权重选择
	totalWeight := 0
	for _, idx := range candidateIdxs {
		totalWeight += accounts[idx].account.EffectiveWeight()
	}
	draw := mathrand.Intn(totalWeight)
	selectedIdx := candidateIdxs[0]
	for _, idx := range candidateIdxs {
		draw -= accounts[idx].account.EffectiveWeight()
		if draw < 0 {
			selectedIdx = idx
			break
		}
...
func selectByWeight(accounts []accountWithLoad) *accountWithLoad {
	...
	for _, item := range accounts {
		total += item.account.EffectiveWeight()
	}
	draw := mathrand.Intn(total)
	for i := range accounts {
		draw -= accounts[i].account.EffectiveWeight()
		if draw < 0 {
			return &accounts[i]
		}
	}
...
func selectGenericLoadAwareCandidate(accounts []accountWithLoad, preferOAuth bool) *accountWithLoad {
	if accountWithLoadWeightsDiffer(accounts) {
		return selectByWeight(accounts)
	}
	return selectByLRU(filterByMinLoadRate(accounts), preferOAuth)
}

func weightedShuffleMinPriorityWithLoad(accounts []accountWithLoad) int {
	if len(accounts) == 0 {
		return 0
	}
	end := 1
	for end < len(accounts) && accounts[end].account.Priority == accounts[0].account.Priority {
		end++
	}
	pool := append([]accountWithLoad(nil), accounts[:end]...)
	for out := 0; len(pool) > 0; out++ {
		total := 0
		...
		selected := 0
		for i := range pool {
			draw -= pool[i].account.EffectiveWeight()
			if draw < 0 {
				selected = i
				break
			}
		}
		accounts[out] = pool[selected]
		pool = append(pool[:selected], pool[selected+1:]...)
	}
	return end
}

func minPriorityWithLoadWeightsDiffer(accounts []accountWithLoad) bool {
	if len(accounts) <= 1 {
		return false
	}
	priority := accounts[0].account.Priority
	weight := accounts[0].account.EffectiveWeight()
	for _, item := range accounts[1:] {
		if item.account.Priority != priority {
			break
		}
		if item.account.EffectiveWeight() != weight {
			return true
		}
	}
	return false
...
func accountWithLoadWeightsDiffer(accounts []accountWithLoad) bool {
	if len(accounts) <= 1 {
		return false
	}
	weight := accounts[0].account.EffectiveWeight()
...
func weightedShuffleMinPriorityAccounts(accounts []*Account) int {
	if len(accounts) == 0 {
		return 0
	}
	end := 1
	for end < len(accounts) && accounts[end].Priority == accounts[0].Priority {
		end++
	}
	pool := append([]*Account(nil), accounts[:end]...)
	for out := 0; len(pool) > 0; out++ {
		total := 0
		for _, account := range pool {
			total += account.EffectiveWeight()
		}
		draw := mathrand.Intn(total)
		selected := 0
		for i := range pool {
			draw -= pool[i].EffectiveWeight()
			if draw < 0 {
				selected = i
				break
			}
...
func minPriorityAccountWeightsDiffer(accounts []*Account) bool {
	if len(accounts) <= 1 {
		return false
	}
	priority := accounts[0].Priority
	weight := accounts[0].EffectiveWeight()
	for _, account := range accounts[1:] {
		if account.Priority != priority {
			break
		}
		if account.EffectiveWeight() != weight {
			return true
		}
	}
	return false
...
func selectMinPriorityByWeightOrLegacy(accounts []*Account, preferOAuth bool, mixedScheduling bool) *Account {
	if len(accounts) == 0 {
		return nil
	}
	...
	// 显式权重有差异时，weight 覆盖 LRU/OAuth 偏好，但绝不跨 priority。
	if accountWeightsDiffer(pool) {
		totalWeight := 0
		for _, account := range pool {
	...
	// 默认权重一致时保持旧版逐个比较语义：未使用 > 最久未使用；仅在
	// LastUsedAt 都为空时应用 Gemini OAuth 偏好。
	selected := pool[0]
	for _, candidate := range pool[1:] {
		...
		case candidate.LastUsedAt == nil && selected.LastUsedAt == nil:
			preferCandidate := preferOAuth && candidate.Type != selected.Type && candidate.Type == AccountTypeOAuth
			if mixedScheduling {
				preferCandidate = preferCandidate && candidate.Platform == PlatformGemini && selected.Platform == PlatformGemini
			}
			if preferCandidate {
				selected = candidate
			}
...
func shuffleWithinSortGroups(accounts []accountWithLoad) {
	...
	for priorityStart := 0; priorityStart < len(accounts); {
		priorityEnd := priorityStart + 1
		for priorityEnd < len(accounts) && accounts[priorityEnd].account.Priority == accounts[priorityStart].account.Priority {
			priorityEnd++
		}
		priorityAccounts := accounts[priorityStart:priorityEnd]
		if accountWithLoadWeightsDiffer(priorityAccounts) {
			weightedShuffleMinPriorityWithLoad(priorityAccounts)
			priorityStart = priorityEnd
			continue
		}
		for i := priorityStart; i < priorityEnd; {
			j := i + 1
			for j < priorityEnd && sameAccountWithLoadGroup(accounts[i], accounts[j]) {
				j++
			}
			if j-i > 1 {
...
func shuffleWithinPriorityAndLastUsed(accounts []*Account, preferOAuth bool) {
	...
	for priorityStart := 0; priorityStart < len(accounts); {
		priorityEnd := priorityStart + 1
		for priorityEnd < len(accounts) && accounts[priorityEnd].Priority == accounts[priorityStart].Priority {
			priorityEnd++
		}
		priorityAccounts := accounts[priorityStart:priorityEnd]
		if accountWeightsDiffer(priorityAccounts) {
			weightedShuffleMinPriorityAccounts(priorityAccounts)
			priorityStart = priorityEnd
			continue
		}
		for i := priorityStart; i < priorityEnd; {
			j := i + 1
			for j < priorityEnd && sameAccountGroup(accounts[i], accounts[j]) {
				j++
			}
			if j-i > 1 {
				if preferOAuth {
...
func shuffleWithinPriority(accounts []*Account) {
	...
	for start < len(accounts) {
		...
		if accountWeightsDiffer(accounts[start:end]) {
			pool := append([]*Account(nil), accounts[start:end]...)
			for out := start; len(pool) > 0; out++ {
				total := 0
				for _, account := range pool {
					total += account.EffectiveWeight()
				}
...
func accountWeightsDiffer(accounts []*Account) bool {
	if len(accounts) <= 1 {
		return false
	}
	weight := accounts[0].EffectiveWeight()
...
func (s *GatewayService) selectAccountForModelWithPlatform(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, platform string) (*Account, error) {
	...
	if len(routingAccountIDs) > 0 {
		...
		selected := selectMinPriorityByWeightOrLegacy(eligibleCandidates, preferOAuth, false)
		if selected != nil {
			if sessionHash != "" && s.cache != nil {
...
func (s *GatewayService) selectAccountWithMixedScheduling(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, nativePlatform string) (*Account, error) {
	...
	if len(routingAccountIDs) > 0 {
		...
		for i := range accounts {
			...
			// 过滤：原生平台、Anthropic 下的 Copilot，以及启用 mixed 的 Antigravity。
		...
		selected := selectMinPriorityByWeightOrLegacy(eligibleCandidates, preferOAuth, true)
		if selected != nil {
			if sessionHash != "" && s.cache != nil {
				if err := s.bindGatewayStickySessionDuringSelection(ctx, groupID, sessionHash, selected.ID); err != nil {
					logger.LegacyPrintf("service.gateway", "set session account failed: session=%s account_id=%d err=%v", sessionHash, selected.ID, err)
