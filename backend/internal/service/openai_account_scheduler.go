...
func buildOpenAIWeightedSelectionOrder(
	...
	for i := range pool {
		...
		weights[i] = weight * float64(pool[i].account.EffectiveWeight())
	}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	buildPrioritySelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
	...
	// priority 是硬层级：只有当前较小 priority 的所有候选均不可用时，
	// 才允许尝试下一层。每层内部再按高级调度 score × 独立 weight 加权，
	// 既保留 DB recheck 的高优先级兜底，又不会让高 priority 账号越级。
	buildSelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
		...
		for start := 0; start < len(ordered); {
			...
			out = append(out, buildPrioritySelectionOrder(ordered[start:end])...)
			start = end
		}

		// Worker 的会话路由会在完成 priority/weight 排序后，将仍在候选集中的
		// 已绑定账号移到首位。这里保留 top-K 保护，但不能让 priority 分层吞掉
		// sticky 命中；否则同一会话会在账号之间抖动。
		if req.StickyWeighted {
			groupTopK := plan.topK
			if groupTopK > len(pool) {
				groupTopK = len(pool)
			...
			for _, stickyID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
				...
				for i, candidate := range out {
					if candidate.account != nil && candidate.account.ID == stickyID {
						promoted := make([]openAIAccountCandidateScore, 0, len(out))
						promoted = append(promoted, candidate)
						promoted = append(promoted, out[:i]...)
						promoted = append(promoted, out[i+1:]...)
						return promoted
					}
				}
			}
		...
		// PreserveStickyBinding 只在当前 sticky 账号触发 TTFT/错误率/并发逃逸时
		// 设置。保留绑定供后续请求复用，但本次应先尝试所有其他候选；若没有
		// 其他可用账号，仍允许最后回退到原账号。
		if req.PreserveStickyBinding && req.StickyAccountID > 0 {
			for i, candidate := range out {
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if skip, _, _ := shouldSkipCopilotAccountForBilling(ctx, account); skip {
		return false, "copilot_billing_credit_limited"
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	for {
		selection, decision, err := s.selectAccountWithSchedulerWithoutRuntimePolicy(ctx, groupID, previousResponseID, sessionHash, requestedModel, localExcluded, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
		if err != nil {
			return selection, decision, err
		}
		if selection == nil || selection.Account == nil {
		...
		if err != nil {
			...
			return nil, decision, fmt.Errorf("acquire account runtime policy: %w", err)
		}
		if gate.Allowed {
...
// selectAccountWithSchedulerWithoutRuntimePolicy wraps selectAccountWithSchedulerOnce with a
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerWithoutRuntimePolicy(
	ctx context.Context,
	groupID *int64,
	previousResponseID string,
	sessionHash string,
...
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResult(accountID int64, model string, success bool, firstTokenMs *int) {
	if !success {
		// 布尔 false 无法表达是否为本地/客户端/模型级错误，默认 neutral。
		// 可归因失败必须调用 ReportOpenAIAccountScheduleOutcome。
		return
	}
	s.ReportOpenAIAccountScheduleOutcome(accountID, model, AccountRuntimeOutcomeHealthy, firstTokenMs)
}

// ReportOpenAIAccountScheduleOutcome updates scheduler/model health for legacy
// callers that only have an account ID. Runtime breaker recording requires the
// request-scoped admission generation and must use one of the typed methods
// below; reloading the account by ID cannot recover that token safely.
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleOutcome(accountID int64, model string, outcome AccountRuntimeOutcome, firstTokenMs *int) {
	if outcome == AccountRuntimeOutcomeNeutral {
		return
	}
	if outcome == AccountRuntimeOutcomeHealthy {
		s.clearOpenAIAccountModelTransientState(accountID, normalizeOpenAIAccountModelTransientModel(model))
	}
	scheduler := s.getOpenAIAccountScheduler(context.Background())
	if scheduler != nil {
		scheduler.ReportResult(accountID, outcome == AccountRuntimeOutcomeHealthy, firstTokenMs)
	}
}

// ReportOpenAIAccountSelectionOutcome records both the request-scoped runtime
// breaker result and the advanced scheduler/model health result.
func (s *OpenAIGatewayService) ReportOpenAIAccountSelectionOutcome(selection *AccountSelectionResult, model string, outcome AccountRuntimeOutcome, firstTokenMs *int) {
	if selection == nil || selection.Account == nil || outcome == AccountRuntimeOutcomeNeutral {
		return
	}
	s.recordAccountRuntimeOutcome(context.Background(), selection.Account, selection.RuntimePolicyGeneration, outcome)
	s.ReportOpenAIAccountScheduleOutcome(selection.Account.ID, model, outcome, firstTokenMs)
}

// ReportOpenAIAccountOutcome is the legacy *Account equivalent. The account
// must be the request-private clone returned by SelectAccountForModel*.
func (s *OpenAIGatewayService) ReportOpenAIAccountOutcome(account *Account, model string, outcome AccountRuntimeOutcome, firstTokenMs *int) {
	if account == nil || outcome == AccountRuntimeOutcomeNeutral {
		return
	}
	s.recordAccountRuntimeOutcome(context.Background(), account, account.runtimePolicyGeneration, outcome)
	s.ReportOpenAIAccountScheduleOutcome(account.ID, model, outcome, firstTokenMs)
}
