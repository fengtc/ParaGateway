...
func TestOpenAISelectAccountWithLoadAwareness_NoSlotFallbackWait(t *testing.T) {
	...
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1, RPMLimit: 120},
		},
	}
	cache := &stubGatewayCache{}
	concurrencyCache := stubConcurrencyCache{
	...
	runtimeStore := &countingAccountRuntimePolicyStore{}
	svc := &OpenAIGatewayService{
		accountRepo:          repo,
		cache:                cache,
		concurrencyService:   NewConcurrencyService(concurrencyCache),
		accountRuntimePolicy: runtimeStore,
	}
	...
	if runtimeStore.acquireCalls != 0 {
		t.Fatalf("wait plan must not consume RPM before a slot is acquired, calls=%d", runtimeStore.acquireCalls)
	}
	gate, err := svc.AcquireAccountSelectionRuntimePolicy(context.Background(), selection)
	if err != nil || !gate.Allowed {
		t.Fatalf("runtime admission after slot failed: gate=%+v err=%v", gate, err)
	}
	if runtimeStore.acquireCalls != 1 {
		t.Fatalf("slot success must consume RPM exactly once, calls=%d", runtimeStore.acquireCalls)
	}
}
