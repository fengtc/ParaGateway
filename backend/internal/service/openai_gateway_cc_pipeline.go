...
func (s *OpenAIGatewayService) openAIChatCompletionsTargetURL(account *Account) (string, error) {
	...
	if account.IsGitHubCopilot() {
		return buildCopilotAPIURL(validatedURL, "/chat/completions"), nil
	}
...
func (s *OpenAIGatewayService) resolveCCFallbackTarget(ctx context.Context, c *gin.Context, account *Account) (apiKey string, targetURL string, err error) {
	if account.IsGitHubCopilot() {
		...
		if strings.TrimSpace(apiKey) == "" {
			return "", "", fmt.Errorf("account %d missing copilot credential", account.ID)
		}
	...
	if strings.TrimSpace(apiKey) == "" {
		return "", "", fmt.Errorf("account %d missing upstream credential", account.ID)
	}
	targetURL, err = s.openAIChatCompletionsTargetURL(account)
	if err != nil {
		return "", "", err
...
func (s *OpenAIGatewayService) sendCCUpstreamRequest(
	...
	return s.sendCCUpstreamRequestWithRetry(ctx, c, account, targetURL, body, stream, bearerToken, userAgent, grokCacheIdentity, true)
}

func (s *OpenAIGatewayService) sendCCUpstreamRequestWithRetry(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	targetURL string,
	...
	if account.IsGitHubCopilot() {
		for key, value := range copilotHeaders(bearerToken, false) {
	...
	if allowCopilotRetry && account.IsGitHubCopilot() && resp.StatusCode == http.StatusUnauthorized && s.openAITokenProvider != nil {
		...
		// A provider may legitimately return the same token when the upstream
		// rejected a stale/cache entry for a transient reason. The 401 itself is
		// the signal to replay once; comparing token strings would incorrectly
		// suppress that retry. The recursive call is bounded by
		// allowCopilotRetry=false.
		if refreshErr == nil && strings.TrimSpace(freshToken) != "" {
			return s.sendCCUpstreamRequestWithRetry(ctx, c, account, targetURL, body, stream, freshToken, userAgent, grokCacheIdentity, false)
		}
