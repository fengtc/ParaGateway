...
type Account struct {
	...
	// runtimePolicyGeneration is request-admission state returned atomically by
	// Redis. It is deliberately neither persisted nor serialized; the exact
	// generation must travel with the selected request to fence stale in-flight
	// results after an administrator changes the breaker policy.
	runtimePolicyGeneration int64
}
...
func (a *Account) EffectiveRPMLimit() int {
	if a == nil || a.RPMLimit < 0 {
		return 0
	}
	return a.RPMLimit
}

func (a *Account) EffectiveCircuitBreakerThreshold() int {
	if a == nil || a.CircuitBreakerThreshold < 0 {
		return 0
	}
	return a.CircuitBreakerThreshold
}

func (a *Account) EffectiveCircuitBreakerCooldown() time.Duration {
	if a == nil || a.CircuitBreakerCooldownSeconds <= 0 {
		return 0
	}
	return time.Duration(a.CircuitBreakerCooldownSeconds) * time.Second
}
...
func (a *Account) IsModelSupported(requestedModel string) bool {
	...
	if len(mapping) == 0 {
		if a.IsGitHubCopilot() {
			// Copilot 是聚合模型目录，合法模型可能来自 OpenAI、Anthropic、
			// Google 等多个家族。空映射沿用“允许所有”语义，不能套用
			// ChatGPT Codex OAuth 的厂商黑名单，否则 gemini-* 等 Copilot
			// 默认模型会在调度阶段被错误排除。
			return true
		}
		if a.IsOpenAIOAuth() {
...
func (a *Account) AllowsOpenAICompact() bool {
	...
	if a.IsGitHubCopilot() {
		// Compact is a ChatGPT/Codex internal protocol. Copilot only exposes
		// the public OpenAI-compatible chat contract.
		return false
	}
...
// IsGitHubCopilot identifies the GitHub Copilot OAuth profile stored in an
// OpenAI-platform account. Copilot uses the OpenAI-compatible gateway contract
// but never uses ChatGPT's Codex endpoints or privacy APIs.
func (a *Account) IsGitHubCopilot() bool {
	return a != nil && a.Platform == PlatformOpenAI && a.Type == AccountTypeOAuth &&
		strings.EqualFold(strings.TrimSpace(a.GetCredential("oauth_profile")), CopilotOAuthProfile)
}
...
// 适用 openai、GitHub Copilot 与国产 OpenAI 兼容供应商
// （kimi/zhipu/deepseek）；grok 走 GetGrokBaseURL，此处对 grok 返回 ""。
...
// GetGitHubAccessToken returns the long-lived GitHub OAuth token used to
// exchange a fresh Copilot token. It is never exposed through DTOs.
func (a *Account) GetGitHubAccessToken() string {
	if !a.IsGitHubCopilot() {
		return ""
	}
	return a.GetCredential("github_access_token")
}
...
func (a *Account) SupportsOpenAIEndpointCapability(capability OpenAIEndpointCapability) bool {
	...
	if a.IsGitHubCopilot() {
		...
		case OpenAIEndpointCapabilityChatCompletions,
			...
			// /v1/responses and /v1/messages are bridged to Copilot's
			// /v1/chat/completions endpoint by the gateway.
			return true
		default:
			// Copilot has no ChatGPT alpha-search, live, embeddings, or
			// image/Responses implementation.
			return false
		}
	}
	switch capability {
