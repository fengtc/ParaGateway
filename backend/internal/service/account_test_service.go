...
// SetOpenAITokenProvider attaches the shared OAuth token lifecycle to manual
// model discovery. It is optional so existing embedded/test constructors keep
// working; Copilot discovery falls back to the persisted token when absent.
func (s *AccountTestService) SetOpenAITokenProvider(provider *OpenAITokenProvider) {
	if s != nil {
		s.openAITokenProvider = provider
	}
}

func (s *AccountTestService) resolveOpenAIAPIKeyToken(ctx context.Context, account *Account) (string, error) {
	if account != nil && account.IsWIF() {
		if s.openAITokenProvider == nil {
			return "", errors.New("WIF token provider is not configured")
		}
	...
	return account.GetOpenAIProtocolAPIKey(), nil
}

func (s *AccountTestService) resolveOpenAIOAuthToken(ctx context.Context, account *Account) (string, error) {
	if account == nil {
		return "", errors.New("account is required")
	}
	if account.IsGitHubCopilot() && s.openAITokenProvider != nil {
		return s.openAITokenProvider.GetAccessToken(ctx, account)
	}
	return account.GetOpenAIAccessToken(), nil
}

func (s *AccountTestService) resolveAnthropicAPIKeyToken(ctx context.Context, account *Account) (string, error) {
	if account != nil && account.IsWIF() {
	...
	return account.GetCredential("api_key"), nil
}
...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	if account.IsOAuth() {
		...
		authToken, err = s.resolveAnthropicAPIKeyToken(ctx, account)
		if err != nil {
			return s.sendErrorAndEnd(c, "Failed to resolve Anthropic credential")
		}
		if authToken == "" {
			return s.sendErrorAndEnd(c, "No API key available")
		}
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if credentialAccount.IsGitHubCopilot() {
		authToken, err := s.resolveOpenAIOAuthToken(ctx, credentialAccount)
		if err != nil {
			return s.sendErrorAndEnd(c, "Failed to resolve Copilot credential")
		}
		if authToken == "" {
			return s.sendErrorAndEnd(c, "No Copilot access token available")
		}
		baseURL := credentialAccount.GetOpenAIBaseURL()
		normalizedBaseURL, err := s.validateUpstreamBaseURL(baseURL)
		if err != nil {
			return s.sendErrorAndEnd(c, fmt.Sprintf("Invalid Copilot base URL: %s", err.Error()))
		}
		return s.testOpenAIChatCompletionsConnection(c, credentialAccount, normalizeCopilotModel(testModelID), prompt, normalizedBaseURL, authToken)
	}
	...
	if credentialAccount.IsOAuth() {
		...
		authToken, err = s.resolveOpenAIAPIKeyToken(ctx, credentialAccount)
		if err != nil {
			return s.sendErrorAndEnd(c, "Failed to resolve OpenAI credential")
		}
		if authToken == "" {
			return s.sendErrorAndEnd(c, "No API key available")
		}
...
func (s *AccountTestService) testOpenAIChatCompletionsConnection(
	...
	if account.IsGitHubCopilot() {
		apiURL = buildCopilotAPIURL(normalizedBaseURL, "/chat/completions")
	}
	...
	if account.IsGitHubCopilot() {
		testModelID = normalizeCopilotModel(testModelID)
	}
	...
	req.Header.Set("Accept", "text/event-stream")
	if account.IsGitHubCopilot() {
		for key, value := range copilotHeaders(authToken, false) {
			req.Header.Set(key, value)
		}
	} else {
		req.Header.Set("Authorization", "Bearer "+authToken)
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		// GitHub Copilot has its own short-lived token and quota lifecycle. Its
		// account test must report upstream failures without applying ChatGPT
		// Codex rate-limit snapshots or permanently disabling the account.
		if !account.IsGitHubCopilot() && resp.StatusCode == http.StatusTooManyRequests {
			s.reconcileOpenAI429State(ctx, account, resp.Header, body)
		}
		if !account.IsGitHubCopilot() && resp.StatusCode == http.StatusUnauthorized && s.accountRepo != nil {
...
func (s *AccountTestService) testOpenAICompactConnection(c *gin.Context, account *Account, testModelID string) error {
	...
	case account.Type == AccountTypeAPIKey:
		authToken, err = s.resolveOpenAIAPIKeyToken(ctx, account)
		if err != nil {
			return s.sendErrorAndEnd(c, "Failed to resolve OpenAI credential")
		}
		if authToken == "" {
