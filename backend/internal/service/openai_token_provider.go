...
type OpenAITokenProvider struct {
	...
	wifTokenProvider   *WIFTokenProvider
...
func NewOpenAITokenProvider(
	...
	return &OpenAITokenProvider{
		...
		openAIOAuthService: openAIOAuthService,
		wifTokenProvider:   nil,
		metrics:            &openAITokenRuntimeMetricsStore{},
		refreshPolicy:      OpenAIProviderRefreshPolicy(),
	}
}

// SetWIFTokenProvider replaces the WIF resolver. Production constructors share
// one memory cache; tests may inject an isolated provider with a fake transport.
func (p *OpenAITokenProvider) SetWIFTokenProvider(provider *WIFTokenProvider) {
	if p != nil {
		p.wifTokenProvider = provider
	}
}
...
func (p *OpenAITokenProvider) GetAccessToken(ctx context.Context, account *Account) (string, error) {
	...
	if account.Platform == PlatformOpenAI && account.Type == AccountTypeAPIKey && account.IsWIF() {
		if p.wifTokenProvider == nil {
			return "", errors.New("WIF token provider is not configured")
		}
		return p.wifTokenProvider.ResolveAccessToken(ctx, account)
	}
	...
	refreshAt := account.GetCredentialAsTime("refresh_at")
	needsRefresh := !account.IsOpenAIPersonalAccessToken() && func() bool {
		if account.IsGitHubCopilot() {
			if refreshAt != nil {
				return time.Until(*refreshAt) <= openAITokenRefreshSkew
			}
			return expiresAt == nil || time.Until(*expiresAt) <= openAITokenRefreshSkew
		}
		return expiresAt == nil || time.Until(*expiresAt) <= openAITokenRefreshSkew
	}()
	if needsRefresh && !account.IsGitHubCopilot() && strings.TrimSpace(account.GetOpenAIRefreshToken()) == "" {
...
// ForceRefresh refreshes an OAuth credential once, regardless of its local
// expiry timestamp. It is used only after a Copilot upstream 401, where the
// provider may revoke a token before the advertised expiry. The common refresh
// API still supplies the local/distributed lock and durable credential write.
func (p *OpenAITokenProvider) ForceRefresh(ctx context.Context, account *Account) (string, error) {
	...
	// Minimal fallback for tests/embedded callers that do not wire OAuthRefreshAPI.
	if p.openAIOAuthService == nil {
		return "", errors.New("openai oauth service is not configured")
	}
	info, err := p.openAIOAuthService.RefreshAccountToken(ctx, account)
	if err != nil {
		return "", err
	}
	credentials := p.openAIOAuthService.BuildAccountCredentials(info)
