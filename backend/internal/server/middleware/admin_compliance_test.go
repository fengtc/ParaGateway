...
func TestAdminComplianceGuardBlocksAdminRouteWhenMissing(t *testing.T) {
	t.Setenv("PARAGATEWAY_DISABLE_ADMIN_COMPLIANCE", "false")
...
func TestAdminComplianceGuardCanBeDisabledForParaGateway(t *testing.T) {
	t.Setenv("PARAGATEWAY_DISABLE_ADMIN_COMPLIANCE", "true")
	gin.SetMode(gin.TestMode)
	svc := service.NewSettingService(&complianceGuardRepoStub{}, &config.Config{})
	router := gin.New()
	router.Use(AdminComplianceGuard(svc))
