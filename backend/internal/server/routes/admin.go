...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		// Worker 风格的独立上游账号管理（不复用 accounts）
		registerUpstreamAccountRoutes(admin, h)
...
func registerUpstreamAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	upstream := admin.Group("/upstream-accounts")
	{
		upstream.GET("", h.Admin.UpstreamAccount.List)
		upstream.POST("", h.Admin.UpstreamAccount.Create)
		upstream.POST("/test-connection", h.Admin.UpstreamAccount.TestDraft)
		upstream.GET("/:id", h.Admin.UpstreamAccount.Get)
		upstream.PUT("/:id", h.Admin.UpstreamAccount.Update)
		upstream.PATCH("/:id/scheduling", h.Admin.UpstreamAccount.SetScheduling)
		upstream.POST("/:id/test-connection", h.Admin.UpstreamAccount.TestSaved)
		upstream.DELETE("/:id", h.Admin.UpstreamAccount.Delete)
	}
}
...
func registerOpenAIOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	// Compatibility aliases for clients that group all provider OAuth flows under
	// one prefix. They intentionally share the same authenticated handlers.
	providerOAuth := admin.Group("/provider-oauth/copilot")
	{
